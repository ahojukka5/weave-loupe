...
- **Report content SHA-256:** `4918932142f56b48c1eba4252ecd065ebd5890529a554690ee2543718b5f6c53`
- **Audit timestamp (UTC):** `2026-07-28T14:21:09+00:00`
- **Re-audit no later than (UTC):** `2026-08-27T14:21:09+00:00`
...
- **Audited source Git SHA:** `ac9bb3d4b9ab2dde21dfffd2b000aadb3ae75e9d`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `ac9bb3d4b9ab2dde21dfffd2b000aadb3ae75e9d`
...
- **LLM max tokens:** `4096`
- **LLM temperature:** `0.0`
- **LLM prompt SHA-256:** `717af8e2c334d484da9f0a8690f14baaded88ffd4946c1f4f14d80d8f74e0ae4`
- **LLM request SHA-256:** `260311c7c239be6092fae14c4f9b3e62c0f385ca4c1073c4454062b01cb46995`
- **Provider-reported model:** `z-ai/glm-5.2`
- **Provider response ID:** `chatcmpl-fb30557c-33c0-483c-bb17-fb07822d70b0`
- **Provider system fingerprint:** `unavailable`
- **Provider finish reason:** `stop`
- **Provider created (Unix):** `1785248470`
- **Provider prompt tokens:** `19263`
- **Provider completion tokens:** `1136`
- **Provider total tokens:** `20399`
- **GitHub run ID:** `30367813562`
- **GitHub workflow SHA:** `77db0e038085c21332a42e0f2b7df71c08b5402c`
...
- `build_manifest` — SHA-256 `a6d40ba0bbd3f8bcbf6cf2d9f8a91f849944ef941db8987484abd0b7b1afce04`
...
- `llvm` — SHA-256 `ea235ea691df3b6dbac36882abbf5438627072fc8dd7a10dce495f52b287b403`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-rKo852/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-u12lm7vk/.audit.loupe.ydrev8tt/artifacts/program",
...
## Summary
The final native program for `docs/audit/fibonacci_runtime.weave` is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The Weave source computes a runtime Fibonacci number based on the `WEAVE_AUDIT_N` environment variable, falling back to 10 if missing, negative, or greater than 46. The WIR accurately reflects this logic. The raw LLVM IR correctly lowers this to valid SSA with proper signed comparisons and memory operations. The optimized LLVM IR successfully inlines the `fib` function, promotes all stack allocations to registers, and preserves the exact source semantics. The final x86_64 assembly is compact, uses correct ABI conventions for `getenv` and `atoi` calls, maintains proper stack alignment, and executes the Fibonacci loop efficiently. All 9 configured runtime matrix cases passed against the exact linked executable, and the native optimization budget contract was fully satisfied without exceeding any limits.

## Verification matrix
- Source semantics and expected result: PASS. The Weave source defines a Fibonacci function and a main function that reads `WEAVE_AUDIT_N`, falling back to 10 if out of range. The runtime matrix in the complete analysis JSON shows all 9 cases passing with expected exit codes (e.g., 55 for input 10, 144 for input 12).
- Weave-to-WIR semantic preservation: PASS. The WIR review projection matches the source structure exactly, preserving the `fib` and `main` functions, control flow, and operations.
- WIR-to-raw-LLVM semantic preservation: PASS. The raw LLVM IR correctly translates WIR operations, using `icmp sle` for `le_i32`, `icmp slt` for `lt_i32`, and `icmp sgt` for `gt_i32`, matching the signed semantics.
- Raw LLVM validity, SSA, types, and control flow: PASS. The raw LLVM IR is valid, uses correct SSA form, has no `undef` or `poison` uses, and control flow branches are well-formed.
- Optimized LLVM semantic preservation: PASS. The optimized LLVM IR inlines `fib` into `main`, promotes stack variables to registers, and preserves the logic. The `freeze i32` instruction correctly prevents poison propagation from `atoi`'s return value.
- Integer signedness, overflow, shifts, and comparisons: PASS. The optimized LLVM IR uses `icmp ugt` for the upper bound check (safe because negative values are already handled by the prior `icmp slt` check in raw LLVM, which was optimized out by combining the checks) and `icmp ult` for the `n <= 1` check. The `add` instructions use `i32` width, matching the source. No overflow issues are present for the constrained input range.
- Calls, return values, ABI, stack alignment, and register use: PASS. The target assembly and disassembly show correct System V AMD64 ABI usage: `leaq` for string pointer, `rdi` for arguments, `callq` for external functions, and `eax` for return values. Stack is 16-byte aligned via `pushq %rbx`.
- Memory safety, lifetime, leaks, and undefined behavior: PASS. The program uses only stack allocations and read-only string literals. No heap allocations are present. The `freeze` instruction in optimized LLVM prevents undefined behavior from potential poison values.
- Target compatibility and native instruction validity: PASS. The target assembly is valid x86_64 instructions. The disassembly confirms proper linking and execution on the target platform (AMD EPYC, Linux, glibc 2.39).
- Native runtime cases and expected observable behavior: PASS. The runtime matrix in the complete analysis JSON shows all 9 cases passing, including edge cases like missing input, zero, negative, and non-numeric input.
- Configured native limits, required call targets, and loop backedges: PASS. The native budget in the complete analysis JSON shows `main` has 25 instructions (max 32), 2 direct calls (max 2), 1 backward conditional branch (min 1, max 1), and required calls to `atoi@plt` and `getenv@plt` are present. No failures recorded.
- Compiler-generated overhead remaining in final native code: PASS. The final native code is highly optimized. The loop is compact with minimal register moves. The only overhead is a `nopw` padding instruction for loop alignment, which is a standard and beneficial optimization for branch prediction.

## Blocking findings
None found.

## Non-blocking opportunities
- The `fib` function in the source uses an iterative approach with three variables (`previous`, `current`, `index`). While the compiler optimizes this efficiently, a source-level algorithm using only two variables could reduce register pressure slightly, though the current code is already well within the optimization budget.
- The raw LLVM IR uses `alloca` and `store`/`load` for local variables, which are completely optimized away in the final code. This is a standard compiler behavior and not a defect.

## Suggested verification
- Add a runtime test case for `WEAVE_AUDIT_N=46` to verify the upper boundary of the Fibonacci calculation directly without the fallback.
- Add a runtime test case with a very large numeric string (e.g., `WEAVE_AUDIT_N=999999999`) to ensure the fallback logic handles potential `atoi` overflow gracefully, though this is a source-level concern rather than a compiler defect.
