...
- **Report content SHA-256:** `1fd70fa6ac55cc10760813b1deede8c172a612fc4e3a2143f261148548d423a6`
- **Audit timestamp (UTC):** `2026-07-31T06:23:51+00:00`
- **Re-audit no later than (UTC):** `2026-08-30T06:23:51+00:00`
...
- **Audited source Git SHA:** `8ed7f023f2c5698d8bc235b2e8b42d02017c36a8`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `8ed7f023f2c5698d8bc235b2e8b42d02017c36a8`
- **Auditor content SHA-256:** `7779111e42f6c0ac2e057a5e2942d459954da3202b86497feb58b365ed2ba4c9`
- **weavec Git SHA:** `3c256165cccd4ab7be1f854824fa2b81a1a233bf`
- **weavec binary SHA-256:** `3e48e742350763dc14592389ebda8486384738e1560a73b60bc17d58564288bb`
- **weavec version:** `weavec v0.3.0+git.3c256165cccd`
...
- **LLM max tokens:** `4096`
- **LLM temperature:** `0.0`
- **LLM prompt SHA-256:** `19bca75efa5dcc5eb051c6ae52ec10c09f7d48d3dcb006abf8d6c6da73295de7`
- **LLM request SHA-256:** `83474886c7174f8b0ffb99f9510d120bb991fa38d197ecd73870205d86401983`
- **Provider-reported model:** `z-ai/glm-5.2`
- **Provider response ID:** `chatcmpl-797e4d97-93da-485a-a06f-8dbf4b38fe33`
- **Provider system fingerprint:** `unavailable`
- **Provider finish reason:** `stop`
- **Provider created (Unix):** `1785479039`
- **Provider prompt tokens:** `19256`
- **Provider completion tokens:** `1345`
- **Provider total tokens:** `20601`
- **GitHub run ID:** `30609408058`
- **GitHub workflow SHA:** `8ed7f023f2c5698d8bc235b2e8b42d02017c36a8`
...
- `build_manifest` — SHA-256 `0c4dbde3db9dd0840a66941264cddf30b465a4fd14787805294cab9faba92ef4`
...
- `llvm` — SHA-256 `7a5e79ed6afc51bafad8ff800fda79db73f22b84667024efee2719d5aaa7e3cb`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-uhHkik/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-qrfcr5q4/.audit.loupe.q2ynnc0w/artifacts/program",
  "sources": [
    "docs/audit/fibonacci_runtime.weave"
  ]
}
...
## Summary
The final native program for `docs/audit/fibonacci_runtime.weave` is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The source semantics are preserved through cleaned WIR, raw LLVM, optimized LLVM, target assembly, and linked executable disassembly. The runtime matrix of 9 cases passes completely, matching expected exit codes. The native optimization budget is respected, with `main` containing 25 instructions (limit 32), 2 direct calls (limit 2), and 1 backward conditional branch (limit 1). The `fib` function is inlined into `main`, and the loop is preserved in both optimized LLVM and final native code as required by the source design.

## Verification matrix
- Source semantics and expected result: PASS. The Weave source defines a Fibonacci loop that reads `WEAVE_AUDIT_N` at runtime, defaults to 10 if missing or out-of-range (0..46), and returns `fib(n)` as the process exit code. The runtime matrix confirms expected exit codes for all 9 cases (e.g., `missing-input-defaults-to-ten` exits 55, `zero` exits 0, `twelve` exits 144).
- Weave-to-WIR semantic preservation: PASS. The WIR projection preserves all source constructs: `getenv`/`atoi` externs, `fib` function with `le_i32` guard and `while` loop, `main` with `ne_ptr` null check, `lt_i32`/`gt_i32` range checks, and `call_i32 fib n`.
- WIR-to-raw-LLVM semantic preservation: PASS. Raw LLVM declares `getenv`/`atoi`, defines `@main` and internal `@fib` with matching control flow: `icmp sle i32 %n, 1` guard, `while.cond1` loop with `add i32` and `icmp sle`, and `main` with `icmp ne ptr`, `icmp slt`, `icmp sgt`.
- Raw LLVM validity, SSA, and control flow: PASS. Raw LLVM uses valid SSA, correct types (`i32`, `ptr`), and well-formed basic blocks with explicit `br` edges. `alloca` instructions are valid and later optimized away.
- Optimized LLVM semantic preservation: PASS. Optimized LLVM inlines `fib` into `main`, promotes stack slots to SSA `phi` nodes, and preserves semantics: `icmp ugt i32 %t3.fr, 46` for range check, `icmp ult i32 %t3.fr, 2` for base case, and `while.body1.i` loop with `add i32` and `icmp sgt`.
- Integer signedness, overflow, shifts, and comparisons: PASS. Comparisons match source signedness: `icmp slt`/`sgt` for `lt_i32`/`gt_i32`, `icmp sle` for `le_i32`. The `add i32` in the loop is safe for n ≤ 46 (max fib(46) = 1836311903 < 2^31). The `icmp ugt`/`ult` in optimized LLVM is a valid equivalence for non-negative values after the `lt_i32 0` check.
- Calls, return values, ABI, stack alignment, and register use: PASS. `main` uses `pushq %rbx` for callee-save preservation, passes `WEAVE_AUDIT_N` string pointer in `%rdi` to `getenv@PLT`, passes `getenv` result in `%rdi` to `atoi@PLT`, and returns `fib(n)` in `%eax` via `retq`. Stack is 16-byte aligned (8 bytes allocated, 8 from return address).
- Memory safety, lifetime, leaks, and undefined behavior: PASS. No dynamic allocation; string literal `@.str0` is a valid null-terminated constant. `getenv` result is null-checked before `atoi` call. No poison or undef uses in raw or optimized LLVM.
- Target compatibility and native instruction validity: PASS. Target assembly specifies `x86_64-pc-linux-gnu` triple and uses valid x86-64 instructions (`leaq`, `callq`, `cmpl`, `jle`, `addl`). Linked disassembly confirms valid ELF64 with standard PLT/GOT stubs.
- Native runtime cases and expected observable behavior: PASS. All 9 runtime cases pass with matching exit codes and empty stdout/stderr. The executable SHA256 in the runtime matrix matches the bundle artifact.
- Configured native limits, required call targets, and loop backedges: PASS. `main` has 25 instructions (max 32), 2 direct calls to `atoi@plt` and `getenv@plt` (required), 1 backward conditional branch (min 1, max 1), 0 indirect calls (max 0), and 1 padding instruction (max 1). Budget `failures` is empty.
- Compiler-generated overhead remaining in final native code: PASS. The loop is compact (7 instructions in `while.body1.i`) with no spills or redundant moves. The `fib` function is inlined, eliminating call overhead. The `nopw` padding is standard alignment and within budget.

## Blocking findings
None found.

## Non-blocking opportunities
- The `nopw` padding at `0x1187` aligns the loop header for branch prediction, which is beneficial and within the padding budget.
- The raw LLVM uses `alloca` for local variables, but these are fully promoted to registers in optimized LLVM and final native code, so no overhead remains.
- The loop could theoretically be vectorized, but the optimization record shows vectorization is not beneficial due to the recurrence in Fibonacci computation. This is an inherent algorithmic limitation, not compiler overhead.

## Suggested verification
- Add a runtime case for `WEAVE_AUDIT_N=46` to verify the upper boundary directly (expected exit code 1836311903 mod 256 = 47, but note that exit codes are 8-bit, so this may require checking the actual behavior).
- Add a runtime case for `WEAVE_AUDIT_N=3` to verify a small non-trivial Fibonacci value (expected exit code 2).
- Verify stack alignment at runtime with a debugger to confirm 16-byte alignment at the `callq` instructions.
