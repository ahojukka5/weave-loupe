...
- **Report content SHA-256:** `29d9fad8681c25f9efc4b15d4510eb446a627748e612136ccf233130992fc9bd`
- **Audit timestamp (UTC):** `2026-07-28T11:26:06+00:00`
- **Re-audit no later than (UTC):** `2026-08-27T11:26:06+00:00`
...
- **Audited source Git SHA:** `4d0bdda5ca2c14122c8c8a3f40febbcb161921c0`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `4d0bdda5ca2c14122c8c8a3f40febbcb161921c0`
- **Auditor content SHA-256:** `2721e4e32b5e836df97ed0eb78c6f42c50d79345b07c9ef8a31b4e132127addc`
...
- **Provider response ID:** `chatcmpl-3260cf6f-5a81-4c85-9fe6-33da5309e0fb`
- **Provider system fingerprint:** `unavailable`
- **GitHub run ID:** `30354675597`
- **GitHub workflow SHA:** `abcd3827cfba0294c0a502f58c0893ede1225dd4`
...
- `build_manifest` — SHA-256 `1636943b624dc4d968c3a759d8b925792d6a4f3a8ba0fc5ca58a9c62c637ebec`
...
- `llvm` — SHA-256 `aa5be4c79fb89daa30bcb1aedce7c08ace23c5c61f705d3aaa684f6b1196cf23`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-9WMcuk/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-6lp_qbwm/.audit.loupe.5gnclamc/artifacts/program",
...
## Summary
The final native program for `docs/audit/fibonacci_runtime.weave` is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The source semantics are preserved through cleaned WIR, raw LLVM, optimized LLVM, target assembly, and linked executable disassembly. The runtime matrix of 9 cases passed perfectly, matching expected exit codes for default, boundary, and invalid inputs. The optimized LLVM successfully inlines `fib` and eliminates all stack traffic, leaving a tight register-only loop in the final native code.

## Verification matrix
- Source semantics and expected result: PASS. Source defines a runtime Fibonacci calculator with fallback to 10 for missing, negative, or >46 inputs. Runtime matrix confirms expected exit codes (e.g., 55 for `n=10`, 144 for `n=12`, 55 for missing input).
- Weave-to-WIR semantic preservation: PASS. WIR projection accurately reflects source logic, including `getenv`/`atoi` calls, boundary checks, and the `fib` loop.
- WIR-to-raw-LLVM semantic preservation: PASS. Raw LLVM IR matches WIR structure, using `alloca`/`store`/`load` for variables and correct `icmp` predicates (`sle`, `ne`, `slt`, `sgt`).
- Raw LLVM validity, SSA, types, and control flow: PASS. Raw LLVM is valid, well-formed SSA, with correct types and explicit control flow branches.
- Optimized LLVM semantic preservation: PASS. Optimized LLVM inlines `fib`, promotes registers, and uses `freeze` on `atoi` result to preserve undefined-behavior safety while maintaining semantics.
- Integer signedness, overflow, shifts, and comparisons: PASS. Comparisons use correct signedness (`slt`, `sgt`, `sle`). The `ugt` in optimized LLVM is valid due to prior `slt` check eliminating negative values. No overflow for `n <= 46`.
- Calls, return values, ABI, stack alignment, and register use: PASS. `getenv` and `atoi` are called via PLT with correct argument registers (`rdi`) and return (`rax`). Stack is 16-byte aligned (`pushq %rbx`).
- Memory safety, lifetime, leaks, and undefined behavior: PASS. No dynamic allocation; string literal is constant. `getenv` null-check prevents null-pointer dereference. `freeze` prevents UB from uninitialized `atoi` paths.
- Target compatibility and native instruction validity: PASS. Target is `x86_64-pc-linux-gnu`. Assembly uses standard instructions, PLT calls, and valid register conventions.
- Native runtime cases and expected observable behavior: PASS. All 9 runtime cases passed, matching expected exit codes and empty stdout/stderr.
- Compiler-generated overhead remaining in final native code: PASS. No overhead remains. Stack traffic from raw LLVM is eliminated. Loop is compact with no spills or redundant instructions.

## Blocking findings
None found.

## Non-blocking opportunities
- The loop in `main` is not vectorized due to a value used outside the loop and uncomputable iteration count, as noted in the optimization record. This is an inherent algorithmic limitation of the iterative Fibonacci sequence, not compiler overhead.
- The `fib` function is completely inlined into `main` in the optimized LLVM, which is optimal for this small program.

## Suggested verification
- Test with `WEAVE_AUDIT_N=46` to verify the upper boundary of the Fibonacci sequence without overflow.
- Test with a large numeric string (e.g., `WEAVE_AUDIT_N=999999999`) to ensure `atoi` overflow does not cause unexpected behavior, though the `> 46` check mitigates this.
