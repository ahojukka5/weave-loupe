...
- **Audit timestamp (UTC):** `2026-07-28T07:33:07+00:00`
- **Re-audit no later than (UTC):** `2026-08-27T07:33:07+00:00`
...
- **Audited source Git SHA:** `8279f7469aa102a488616d5b26a4ff9e7eb4f213`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `8279f7469aa102a488616d5b26a4ff9e7eb4f213`
- **Auditor content SHA-256:** `e59fcb731cf7d91bb1295e1908844cd6ca95977ddfb49ff5d49a73bef9941b50`
...
- **GitHub run ID:** `30338703388`
- **GitHub workflow SHA:** `c6fd13b964485025f028650f222efb7002aa852f`
...
- Runtime matrix `docs/audit/fibonacci_runtime.audit.json` — SHA-256 `55704fe7fc8b2769103aa0eff469659defd1f1a956a8f2b790f2b4165723471f`

## Captured evidence

- `assembly` — SHA-256 `355551c6d11759a2137d794f6154bee9f187aff00d09f7f13887d82a052f3bf1`
- `build_manifest` — SHA-256 `db902094654961386e5ee22613253191d2eaf2058fa71f2dc2c2a08e6da3f373`
...
- `llvm` — SHA-256 `e01275a369a851a234852698f6bf958a476a312b968de9a7993aaa719542dcb7`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-8FdqSs/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-0c8zrrra/.audit.loupe.kb5r44qk/artifacts/program",
...
## Summary
The final native program is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The Weave source implements a runtime Fibonacci calculator that reads `WEAVE_AUDIT_N` from the environment, falls back to 10 for missing, negative, or out-of-range input, and computes `fib(n)` iteratively. The cleaned WIR preserves these semantics exactly. Raw LLVM IR is valid SSA with correct control flow and signed integer handling. Optimized LLVM inlines `fib` into `main`, promotes all stack slots to registers, and preserves the input-dependent loop as required. The final x86_64 assembly uses a compact register-allocated loop with correct signed comparisons and ABI-compliant calls to `getenv` and `atoi`. All 9 configured runtime cases pass against the hash-addressed sidecar, confirming end-to-end correctness.

## Verification matrix

- **Source semantics and expected result**: PASS. Source (`docs/audit/fibonacci_runtime.weave`) defines `fib(n)` returning `n` for `n <= 1`, otherwise iterating from index 2 to `n` with `previous=0, current=1`. `main` reads `WEAVE_AUDIT_N`, defaults to 10, clamps negative or >46 values to 10, and returns `fib(n)`. Runtime cases confirm expected exit codes: missing→55, 0→0, 1→1, 2→1, 10→55, 12→144, -1→55, 47→55, "abc"→0.

- **Weave-to-WIR semantic preservation**: PASS. Cleaned WIR matches source: `fib` has `le_i32 n (const_i32 1)` guard, `let previous/current/index`, `while (le_i32 index n)`, `add_i32` for `next`, and correct `set` ordering. `main` has `call_ptr getenv`, `ne_ptr input (const_null)`, `call_i32 atoi`, `lt_i32 n 0` and `gt_i32 n 46` clamps, and `call_i32 fib n` return.

- **WIR-to-raw-LLVM semantic preservation**: PASS. Raw LLVM `@main` allocas `%n.addr`, calls `@getenv`, stores 10, checks `icmp ne ptr %t1, null`, calls `@atoi`, checks `icmp slt i32 %t4, 0` and `icmp sgt i32 %t6, 46`, stores 10 on each branch, loads `%n.addr` and calls `@fib`. `@fib` checks `icmp sle i32 %n, 1`, returns `n`, otherwise loops with `add i32` and `icmp sle` condition. All types and operations match WIR.

- **Raw LLVM validity, SSA, types, and control flow**: PASS. Both functions are valid SSA. `@main` has 7 basic blocks with proper branching and domination. `@fib` has 4 basic blocks with loop back-edge from `while.body1` to `while.cond1`. All `%t` values are numbered uniquely, loads/stores use `ptr` allocas, `icmp` predicates are correct. No `undef` or `poison` uses (analysis JSON: `undef_uses: 0, poison_uses: 0`).

- **Optimized LLVM semantic preservation**: PASS. Optimized LLVM inlines `fib` into `main` (optimization record confirms `Inlined` with cost -15005). Stack allocas eliminated (`alloca: 0`). `%t3.fr = freeze i32 %t3` prevents UB from `atoi` returning undef. Range check uses `icmp ugt i32 %t3.fr, 46` (equivalent to signed `> 46` for values that `atoi` can produce in practice, and the freeze ensures determinism). Loop uses `phi` nodes for `index`, `current`, `previous` with correct initial values and updates. `fib.exit` returns `%t3.fr` for `n < 2` or `%t5.i` for loop result.

- **Integer signedness, overflow, shifts, and comparisons**: PASS. Source uses `i32` throughout. Raw LLVM uses `icmp sle/slt/sgt` (signed) for all comparisons matching `le_i32/lt_i32/gt_i32`. `add i32` is used for `next` and `index` increment. For `n` in 0..46, `fib(n)` max is `fib(46) = 1836311903` which fits in signed `i32` (max 2147483647), so no overflow. Optimized LLVM's `icmp ult i32 %t3.fr, 2` for the `n <= 1` check is equivalent to signed `< 2` because the preceding `ugt 46` check already rejected values > 46, and `freeze` ensures no undef. Native code uses `cmpl $46, %eax; ja` (unsigned above) and `cmpl $2, %eax; jb` (unsigned below) matching the optimized LLVM, and `cmpl %ebx, %ecx; jle` (signed less-or-equal) for the loop condition.

- **Calls, return values, ABI, stack alignment, and register use**: PASS. `main` is `i32 @main()` returning in `%eax` per System V AMD64. `getenv` and `atoi` are called with `ptr` argument in `%rdi` (assembly: `leaq .L.str0(%rip), %rdi; callq getenv@PLT` and `movq %rax, %rdi; callq atoi@PLT`). `%rbx` is callee-saved, pushed in prologue (`pushq %rbx`) and popped before return (`popq %rbx; retq`), holding `n` across calls. Stack is 8 bytes (optimization record: `NumStackBytes: 8`), 16-byte aligned at call sites per ABI. Return value in `%eax`/`%ebx` matches all runtime exit codes.

- **Memory safety, lifetime, leaks, and undefined behavior**: PASS. No heap allocation. Stack allocas in raw LLVM are properly scoped and eliminated in optimized LLVM. `freeze i32 %t3` in optimized LLVM prevents UB from `atoi` returning undef on non-numeric input (runtime case "abc" returns 0, matching `atoi("abc")=0`). No null pointer dereference: `getenv` result is checked with `icmp ne ptr %t1, null` before passing to `atoi`. String literal `@.str0` is null-terminated (`c"WEAVE_AUDIT_N\00"`).

- **Target compatibility and native instruction validity**: PASS. Target triple `x86_64-pc-linux-gnu` matches machine (`x86_64`, `glibc 2.39`). Assembly uses valid x86_64 instructions: `pushq`, `leaq`, `callq`, `movl`, `testq`, `cmpl`, `ja`, `jb`, `jle`, `addl`, `incl`, `xorl`, `popq`, `retq`. PLT stubs for `getenv` and `atoi` are standard glibc dynamic linking. `.note.GNU-stack` section present (no executable stack). `_start` calls `main` via `__libc_start_main`.

- **Native runtime cases and expected observable behavior**: PASS. All 9 cases pass (analysis JSON: `runtime.passed: true`). Cases cover missing input (exit 55), 0 (exit 0), 1 (exit 1), 2 (exit 1), 10 (exit 55), 12 (exit 144), -1 (exit 55), 47 (exit 55), "abc" (exit 0). Executable SHA256 `8096a9e8...` matches between bundle and runtime matrix. Sidecar hash `55704fe7...` matches metadata.

- **Compiler-generated overhead remaining in final native code**: PASS. The input-dependent Fibonacci loop remains in optimized LLVM and native code as required by the source design (no constant folding since `n` comes from `getenv`/`atoi`). The loop is 6 instructions (`movl`, `movl`, `addl`, `incl`, `movl`, `cmpl`, `jle`) with no spills, no redundant moves, no stack traffic inside the loop. Register allocation uses `%eax` (current), `%edx` (previous), `%ecx` (index), `%ebx` (n), `%esi` (temp). The only overhead is the single `pushq %rbx`/`popq %rbx` pair for callee-saved register preservation, which is mandatory per ABI and not avoidable. Loop vectorization was attempted but correctly rejected (optimization record: `NonReductionValueUsedOutsideLoop`, `CantComputeNumberOfIterations`) — this is a source algorithm characteristic, not compiler overhead.

## Blocking findings
None found.

## Non-blocking opportunities
- The `fib` function could be optimized to a closed-form Binet formula or matrix exponentiation for large `n`, but the source explicitly requires an input-dependent loop to remain, and the current iterative approach is the intended algorithm. This is a source-level algorithm choice, not compiler overhead.
- The `freeze i32 %t3` in optimized LLVM adds one instruction versus raw `atoi` output, but this is a correctness requirement to avoid UB from `atoi` returning undef on non-numeric input, not avoidable overhead.
- The `cmpl $46, %eax; ja` uses unsigned comparison instead of signed; this is semantically equivalent given the preceding `freeze` and the fact that `atoi` on valid input in 0..46 cannot produce negative values that reach this check (negative values are handled by the `lt_i32 n 0` check in source, which the optimizer folded into the unsigned range check). This is a valid optimization, not a defect.

## Suggested verification
- Add a runtime case with `WEAVE_AUDIT_N=46` to verify `fib(46)=1836311903` fits in `i32` and returns correctly (exit code would be `1836311903 & 0xFF = 255`, which may need special handling in the sidecar).
- Add a runtime case with `WEAVE_AUDIT_N=3` to verify `fib(3)=2` (exit code 2).
- Add a runtime case with a very large numeric string like `WEAVE_AUDIT_N=999999999` to verify the `> 46` clamp works for values exceeding `i32` range (though `atoi` behavior on overflow is implementation-defined).
- Consider adding a stress test that runs the loop many times to verify no stack leakage or register corruption across iterations.
