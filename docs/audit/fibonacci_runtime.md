...
- **Audit timestamp (UTC):** `2026-07-28T04:08:28+00:00`
- **Re-audit no later than (UTC):** `2026-08-27T04:08:28+00:00`
...
- **Audited source Git SHA:** `62e7c65a7546b828488b13e6844e894c258522cb`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `62e7c65a7546b828488b13e6844e894c258522cb`
- **Auditor content SHA-256:** `f41971ca449e12aaf8fdc28e7a84cc33b48d85c1856a7d5f82de4a185b7212d6`
...
- **GitHub run ID:** `30327823805`
- **GitHub workflow SHA:** `12e7fdd98ca910ee220e4c00c416ec949ef5d390`
...
- `build_manifest` — SHA-256 `94de4ff2132ce21fe80c78d7cbcbc4108425a1babe57859ffab0762417aaaa73`
...
- `llvm` — SHA-256 `e093424bba4f68d8052bbf9a518a98930cbce8545cd63aa4baeca1acc27e26cf`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-BwVTZE/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-43e7vqnf/.audit.loupe.irlf95n0/artifacts/program",
...
## Summary
The final native program for `docs/audit/fibonacci_runtime.weave` is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The Weave source defines a runtime-dependent Fibonacci computation that reads `WEAVE_AUDIT_N`, clamps the input to the range `[0, 46]` with a fallback of 10, and returns `fib(n)`. The cleaned WIR preserves these semantics exactly. The raw LLVM IR is valid SSA with correct control flow and memory operations. During LLVM optimization, the internal `fib` function is inlined into `main`, its stack allocations are promoted to registers, and the input clamping logic is combined into a single unsigned comparison (`icmp ugt i32 %t3.fr, 46`), which is a valid optimization because negative `atoi` results map to large unsigned values. The final x86_64 assembly correctly implements this logic, uses a tight loop for the Fibonacci calculation, and adheres to the System V AMD64 ABI. All 9 configured runtime cases pass, confirming the end-to-end correctness of the exact linked artifact.

## Verification matrix
- Source semantics and expected result: PASS. The Weave source defines `fib` and `main` with input clamping. Expected results are confirmed by the 9 passing runtime cases in the complete analysis JSON.
- Weave-to-WIR semantic preservation: PASS. The cleaned WIR projection matches the source's `if`, `while`, `let`, `set`, and `call` structure without semantic alteration.
- WIR-to-raw-LLVM semantic preservation: PASS. The raw LLVM IR implements the WIR structure directly using `alloca`, `load`, `store`, and `br` instructions corresponding to the WIR statements.
- Raw LLVM validity, SSA, types, and control flow: PASS. The raw LLVM IR uses valid SSA values, correct types (`i32`, `ptr`), and well-formed basic blocks with explicit `br` and `ret` terminators.
- Optimized LLVM semantic preservation: PASS. The optimized LLVM IR inlines `fib`, promotes memory to SSA values, and combines the clamping checks. The `icmp ugt` correctly handles negative inputs by treating them as large unsigned values, preserving the source's fallback logic.
- Integer signedness, overflow, shifts, and comparisons: PASS. The `add i32` operations in the Fibonacci loop match the source's `add_i32`. The `ult` and `ugt` comparisons in optimized LLVM correctly implement the source's signed bounds checks by exploiting the unsigned representation of the values.
- Calls, return values, ABI, stack alignment, and register use: PASS. The target assembly uses `leaq`/`callq` for `getenv` and `atoi`, passes the string pointer in `%rdi`, receives the result in `%rax`, and returns the final value in `%eax`. `pushq %rbx` maintains 16-byte stack alignment.
- Memory safety, lifetime, leaks, and undefined behavior: PASS. The program uses only stack memory and read-only data. The optimized LLVM IR contains no `alloca`, `load`, or `store`. No undefined behavior or memory leaks are present.
- Target compatibility and native instruction validity: PASS. The linked executable disassembly shows valid x86_64 instructions (`leaq`, `callq`, `cmpl`, `ja`, `jb`, `jle`, `addl`, `incl`) and standard ELF structure (`_start`, PLT, `.init`, `.fini`).
- Native runtime cases and expected observable behavior: PASS. The complete analysis JSON shows 9 configured runtime cases, all with `"passed": true` and no `"failures"`. The executable SHA256 in the runtime matrix matches the bundle artifact.
- Compiler-generated overhead remaining in final native code: PASS. The final native code is highly compact (25 instructions in `main`). The loop is tight and uses only registers. The only overhead noted in the optimization record is a loop-carried copy (`movl %eax, %esi`), which is an unavoidable artifact of the x86 two-operand instruction format and not a compiler-generated inefficiency.

## Blocking findings
None found.

## Non-blocking opportunities
- The Fibonacci loop in the final assembly uses a loop-carried copy (`movl %eax, %esi` at `0x1190`) due to the two-operand nature of x86 `addl`. This is a target architecture limitation, not a compiler-generated overhead defect.
- The loop-vectorize and slp-vectorizer passes missed vectorizing the Fibonacci loop, as noted in the optimization record. This is expected because the Fibonacci recurrence is inherently sequential and not vectorizable.

## Suggested verification
- Add a runtime case for `WEAVE_AUDIT_N=46` to explicitly test the upper boundary of the Fibonacci computation without triggering the fallback.
- Add a runtime case for a very large positive integer (e.g., `WEAVE_AUDIT_N=100`) to further confirm the `> 46` clamping logic.
