...
- **Report content SHA-256:** `93a8d0fe0da88b1f99037948d4df8dd891a7319969891fddb125f6acb25a6ad6`
- **Audit timestamp (UTC):** `2026-07-28T13:52:55+00:00`
- **Re-audit no later than (UTC):** `2026-08-27T13:52:55+00:00`
...
- **Audited source Git SHA:** `d22ffc5687adeea2fe76c161510930bfdf4bbd4c`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `d22ffc5687adeea2fe76c161510930bfdf4bbd4c`
- **Auditor content SHA-256:** `c955ece8bc3cb6312fb03020593407eccfd957581a21387c306d9c3305f2801b`
...
- **LLM max tokens:** `4096`
- **LLM temperature:** `0.0`
- **LLM prompt SHA-256:** `05c0634597697ae23cd675a8f12b7bb07fa8ae324b18cd297199ed727dede151`
- **LLM request SHA-256:** `b1da565a62ad139b6cd4f803ce708f7340e3d7d0de95446d5156cc77e242fa9e`
- **Provider-reported model:** `z-ai/glm-5.2`
- **Provider response ID:** `chatcmpl-f300799a-8064-446e-ae0b-833798d2a7a0`
- **Provider system fingerprint:** `unavailable`
- **Provider finish reason:** `stop`
- **Provider created (Unix):** `1785246776`
- **Provider prompt tokens:** `18810`
- **Provider completion tokens:** `1016`
- **Provider total tokens:** `19826`
- **GitHub run ID:** `30365504440`
- **GitHub workflow SHA:** `43e51c7d4b676f9d1fa1eb486b3c01cd7b083953`
...
- `build_manifest` — SHA-256 `0467a96616bd6297892e213dc70293ab9893a306391f872f0d46a2ee528c60c9`
...
- `llvm` — SHA-256 `be2cbe7678a7f623a04edaec0c06aa27b7af0c0bfa82f2d4ff4ae570e384d50e`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-lEAXCl/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-lrdskxqy/.audit.loupe.bgaeoiod/artifacts/program",
...
## Summary
The final native program for `fibonacci_runtime.weave` is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The Weave source semantics are preserved through cleaned WIR, raw LLVM, optimized LLVM, target assembly, and linked executable disassembly. The runtime matrix of 9 cases passed completely, matching expected exit codes. The native optimization budget was respected, with `main` observing 25 instructions against a limit of 32. No merge-blocking defects were found.

## Verification matrix
- Source semantics and expected result: PASS. The Weave source defines a runtime Fibonacci calculator bounded to 0..46, defaulting to 10 on missing/invalid input. The runtime matrix in the complete analysis JSON confirms all 9 cases (e.g., `ten` -> 55, `twelve` -> 144, `negative-falls-back` -> 55) match expected exit codes.
- Weave-to-WIR semantic preservation: PASS. The WIR review projection preserves all source logic, including `getenv`/`atoi` externs, `fib` loop, and `main` input bounds checking.
- WIR-to-raw-LLVM semantic preservation: PASS. Raw LLVM IR implements the WIR exactly, including `icmp` conditions and `while` loop control flow.
- Raw LLVM validity, SSA, types, and control flow: PASS. The raw LLVM IR is valid, uses correct SSA form, and has no `undef` or `poison` uses according to the complete analysis JSON.
- Optimized LLVM semantic preservation: PASS. The optimized LLVM inlines `fib` into `main`, promotes stack variables to SSA registers, and preserves the `freeze i32` on `atoi`'s return to prevent undefined behavior.
- Integer signedness, overflow, shifts, and comparisons: PASS. The optimized LLVM uses `icmp ugt` for the upper bound (safe because negative `atoi` results are handled) and `icmp ult` for the lower bound. The assembly uses `cmpl $46, %eax` followed by `ja` (unsigned above) and `cmpl $2, %eax` followed by `jb` (unsigned below), matching the optimized LLVM.
- Calls, return values, ABI, stack alignment, and register use: PASS. The assembly follows the System V AMD64 ABI: `leaq .L.str0(%rip), %rdi` passes the string to `getenv`, `movq %rax, %rdi` passes the result to `atoi`, and `pushq %rbx` / `popq %rbx` preserves the callee-saved register across the calls.
- Memory safety, lifetime, leaks, and undefined behavior: PASS. The program uses only stack allocations and read-only string literals. No heap allocations or leaks exist. The `freeze` instruction prevents UB from uninitialized memory.
- Target compatibility and native instruction validity: PASS. The linked executable disassembly shows valid x86_64 instructions (e.g., `endbr64`, `callq`, `retq`) and standard ELF64 structure (`.init`, `.plt`, `.text`, `.fini`).
- Native runtime cases and expected observable behavior: PASS. The runtime matrix in the complete analysis JSON shows 9/9 cases passed with 0 failures, matching expected exit codes and empty stdout/stderr.
- Configured native optimization budget and observed final-code metrics: PASS. The native budget in the complete analysis JSON shows `main` observed 25 instructions (limit 32), 2 direct calls (limit 2), and 1 padding instruction (limit 1). The budget passed.
- Compiler-generated overhead remaining in final native code: PASS. The final assembly is compact, with no redundant spills, zero-idiom instructions (`xorl %edx, %edx`), or unnecessary moves. The loop is tight and uses registers efficiently.

## Blocking findings
None found.

## Non-blocking opportunities
- The `fib` loop could not be vectorized due to non-reduction value usage and unknown iteration count, as noted in the optimization record. This is a source algorithm cost (inherent loop-carried dependency) and not compiler-generated overhead.
- The `nopw` padding instruction in `main` aligns the loop header for performance but counts against the padding instruction budget. It is within limits and beneficial.

## Suggested verification
- Add a runtime case for `WEAVE_AUDIT_N=46` to verify the upper boundary of the Fibonacci calculation (expected exit code 1836311903).
- Add a runtime case for `WEAVE_AUDIT_N=3` to verify the loop executes correctly for small values (expected exit code 2).
- Add a runtime case for `WEAVE_AUDIT_N=` (empty string) to verify `atoi` returns 0 and the program defaults to 0 (expected exit code 0).
