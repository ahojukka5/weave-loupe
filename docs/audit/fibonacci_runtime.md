...
- **Report content SHA-256:** `7106d6937115407d01393a67d6dd7161e906160fc2c88e3b632de569a03f1687`
- **Audit timestamp (UTC):** `2026-07-28T11:41:42+00:00`
- **Re-audit no later than (UTC):** `2026-08-27T11:41:42+00:00`
...
- **Audited source Git SHA:** `fe207a32012caae429d064bb26b9ffa067f84a26`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `fe207a32012caae429d064bb26b9ffa067f84a26`
- **Auditor content SHA-256:** `17c4a4dc60010851f33f87b23bcd12db89a46a0bf583c78ac6d0a0492ccdfcae`
...
- **Provider response ID:** `chatcmpl-33e7536b-0ff7-4d63-89f9-eb9f1443555c`
- **Provider system fingerprint:** `unavailable`
- **GitHub run ID:** `30355684794`
- **GitHub workflow SHA:** `1ba8003a3bbec52f7060d2a14c58848f79becf40`
...
- `build_manifest` — SHA-256 `9a1591312f8703148b445896ca19b9ed94d768f8667a845add45c6e80e85f618`
...
- `llvm` — SHA-256 `acad995a1b3548df59cc8aac04a256ac3e8a924759e9cddd4470e9da9baf49e5`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-t2Dlo8/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-6dl9r1s9/.audit.loupe.yfhvkzuh/artifacts/program",
...
## Summary
The final native program for `fibonacci-runtime` is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The Weave source semantics are preserved through the WIR and raw LLVM IR. The LLVM optimizer successfully inlines the `fib` function and promotes stack allocations to registers, eliminating all raw LLVM stack traffic. The final x86_64 assembly correctly implements the runtime logic, using signed comparisons where required and adhering to the System V AMD64 ABI. All 9 configured runtime matrix cases pass, providing direct evidence of the linked artifact's correctness.

## Verification matrix
- Source semantics and expected result: PASS. The source defines a program that reads `WEAVE_AUDIT_N`, defaults to 10 if missing, negative, or > 46, and computes Fibonacci(n) for n in 0..46. The runtime matrix confirms these exact behaviors across 9 cases.
- Weave-to-WIR semantic preservation: PASS. The WIR projection matches the source structure, preserving the `getenv`/`atoi` calls, bounds checks, and the `fib` loop logic.
- WIR-to-raw-LLVM semantic preservation: PASS. The raw LLVM IR correctly translates the WIR, using `alloca` for variables and implementing the control flow with `icmp` and `br` instructions.
- Raw LLVM validity, SSA, types, and control flow: PASS. The raw LLVM IR is well-formed, uses valid SSA, correct types, and has valid control flow edges.
- Optimized LLVM semantic preservation: PASS. The optimized LLVM IR inlines `fib` into `main`, promotes all variables to registers (0 `alloca`/`store`/`load`), and preserves the signed bounds checks and loop logic.
- Integer signedness, overflow, shifts, and comparisons: PASS. The optimized LLVM uses `icmp ugt i32 %t3.fr, 46` for the upper bound check, which is safe because negative `atoi` results map to large unsigned values and correctly fall back to 10. The `fib` loop uses `icmp sgt` for the loop exit condition, matching the signed semantics.
- Calls, return values, ABI, stack alignment, and register use: PASS. The native code uses `rdi` for string arguments, calls `getenv@PLT` and `atoi@PLT`, and returns the result in `eax`. The stack is 16-byte aligned (`pushq %rbx` followed by `retq`).
- Memory safety, lifetime, leaks, and undefined behavior: PASS. The program performs no manual memory allocation. The `getenv` pointer is checked for null before being passed to `atoi`. No undefined behavior is present.
- Target compatibility and native instruction validity: PASS. The linked executable disassembly shows valid x86_64 instructions, correct PLT stubs, and standard CRT initialization functions.
- Native runtime cases and expected observable behavior: PASS. All 9 runtime cases pass, including edge cases like missing input, negative input, non-numeric input, and boundary values.
- Compiler-generated overhead remaining in final native code: PASS. The final native code contains no compiler-generated overhead. The `fib` loop is compact, and all raw LLVM stack traffic was optimized away.

## Blocking findings
None found.

## Non-blocking opportunities
- The `fib` loop in the native code uses a sequence of `mov` instructions to shuffle registers (`movl %eax, %esi`, `movl %edx, %eax`, `movl %esi, %edx`). While this is standard for a Fibonacci recurrence, a source-level algorithm change or target-specific register allocation could potentially reduce these moves, though this is a source algorithm cost, not compiler overhead.
- The loop-vectorize and slp-vectorizer passes missed vectorization opportunities, which is expected and non-blocking for this sequential recurrence.

## Suggested verification
- Add a runtime case for `WEAVE_AUDIT_N=46` to explicitly test the upper boundary of the valid range.
- Add a runtime case for a very large positive number (e.g., `WEAVE_AUDIT_N=100`) to ensure the fallback logic is robust against integer overflow in `atoi`.
