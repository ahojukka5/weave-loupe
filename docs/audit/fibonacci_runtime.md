...
- **Audit timestamp (UTC):** `2026-07-28T03:28:00+00:00`
- **Re-audit no later than (UTC):** `2026-08-27T03:28:00+00:00`
...
- **Audited source Git SHA:** `1dfa0fa5ba7fd50d27c0c5aef69b812778131275`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `1dfa0fa5ba7fd50d27c0c5aef69b812778131275`
...
- **GitHub run ID:** `30325883887`
- **GitHub workflow SHA:** `97788da38b32f8e3b7ccb7bb6a83ed020659769b`
...
- `build_manifest` — SHA-256 `1bc12db7d9abcd77783f4e08278bba5d22cf12e58e551af2a127bc10c6071416`
...
- `llvm` — SHA-256 `38d7d55b80ebece32c0b6dfdd4de7cf6f259028b70cf5497c2573ec3eda808b4`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-02FR5F/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-y1ov5kqp/.audit.loupe.nj2uzw3a/artifacts/program",
...
## Summary
The final native program is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The Weave source defines a runtime-driven Fibonacci computation with input clamping, and the cleaned WIR preserves those semantics exactly. Raw LLVM IR is valid SSA with correct control flow, and the optimized LLVM IR preserves the source semantics through proper inlining and promotion of stack slots to SSA values. The final x86_64 assembly is valid, uses the correct System V ABI for external calls, and maintains proper stack alignment. The runtime matrix of 9 cases all passed, confirming the linked executable's observable behavior matches the expected sidecar values.

## Verification matrix
- Source semantics and expected result: PASS. The Weave source defines `fib` to return `n` for `n <= 1`, otherwise iteratively compute Fibonacci. `main` reads `WEAVE_AUDIT_N`, defaults to 10 if missing, clamps negative or >46 values to 10, and returns `fib(n)`. Runtime cases confirm these semantics.
- Weave-to-WIR semantic preservation: PASS. The cleaned WIR matches the source structure exactly, preserving the `fib` logic, `main` control flow, and external calls to `getenv` and `atoi`.
- WIR-to-raw-LLVM semantic preservation: PASS. Raw LLVM IR directly corresponds to WIR, using `alloca` for local variables, `icmp` for conditions, and `br` for control flow as expected.
- Raw LLVM validity, SSA, and control flow: PASS. The raw LLVM IR is valid SSA with proper typing, well-formed basic blocks, and correct branching.
- Optimized LLVM semantic preservation: PASS. The optimized LLVM IR inlines `fib` into `main`, promotes stack slots to SSA values, and preserves the input clamping logic and loop semantics.
- Integer signedness, overflow, shifts, and comparisons: PASS. The source limits `n` to 0..46, preventing `i32` overflow in `fib` (max value 1,836,311,903). Comparisons use correct signedness (`slt`, `sgt`, `sle` in LLVM; `jle`, `jb`, `ja` in native code).
- Calls, return values, ABI, stack alignment, and register use: PASS. `getenv` and `atoi` are called with arguments in `%rdi`, return values in `%rax`, and `main` returns `i32` in `%eax`. `pushq %rbx` maintains 16-byte stack alignment.
- Memory safety, lifetime, leaks, and undefined behavior: PASS. No dynamic allocation is used. The string literal is properly null-terminated. The `freeze` instruction in optimized LLVM prevents undefined behavior from `atoi` returning undef.
- Target compatibility and native instruction validity: PASS. The target assembly and disassembly are valid x86_64 instructions, matching the `x86_64-pc-linux-gnu` triple and the AMD EPYC 9V74 machine.
- Native runtime cases and expected observable behavior: PASS. All 9 runtime cases passed, with exit codes matching expected values (e.g., 55 for `n=10`, 144 for `n=12`, 0 for `n=0`).
- Compiler-generated overhead remaining in final native code: PASS. The final native code is compact (25 instructions in `main`), with no redundant stack traffic or unnecessary instructions. The loop is efficiently implemented using registers.

## Blocking findings
None found.

## Non-blocking opportunities
- The loop-vectorize and SLP-vectorizer passes missed optimization opportunities, as noted in the optimization record. This is non-blocking because the loop is inherently sequential and not beneficial to vectorize.
- The raw LLVM IR uses `alloca` for local variables, which is optimized away in the optimized LLVM IR. This is non-blocking as it is a standard compiler intermediate representation step.

## Suggested verification
- Add a runtime case for `WEAVE_AUDIT_N=46` to explicitly verify the upper boundary of the Fibonacci computation without clamping.
- Add a runtime case for `WEAVE_AUDIT_N=2` to verify the loop executes exactly once and returns 1.
- Add a runtime case for a large numeric string (e.g., `WEAVE_AUDIT_N=999999999999999999`) to verify `atoi` overflow behavior and subsequent clamping.
