...
- **Audit timestamp (UTC):** `2026-07-28T00:09:59+00:00`
- **Re-audit no later than (UTC):** `2026-08-27T00:09:59+00:00`
...
- **Audited source Git SHA:** `5cda4672d6473cd6d8dc9c1235f0c03b7801914e`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `5cda4672d6473cd6d8dc9c1235f0c03b7801914e`
...
- **GitHub run ID:** `30316414371`
- **GitHub workflow SHA:** `fec762d4e50acfd59c006bee4f8c76a9743ddaff`
...
- `build_manifest` — SHA-256 `4099b524a3e0f5b36fee4b2e8e308e62758514175776152711dc1267df889a85`
...
- `llvm` — SHA-256 `5ce7aabc92f8b2be8320d301a241dbfdfac2f5174d75d49cb80539a6143d755a`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-MvvJfn/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-marztjgc/.audit.loupe.h_ipp6_u/artifacts/program",
...
## Summary
The final native program for `docs/audit/fibonacci_runtime.weave` is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The source semantics are preserved through cleaned WIR, raw LLVM, optimized LLVM, target assembly, and the linked executable disassembly. The runtime matrix, sourced from a hash-addressed sidecar, passes all 9 configured cases. The optimized LLVM successfully inlines `fib` into `main`, promotes stack locals to SSA registers, and eliminates all `alloca`/`load`/`store` traffic. The final x86_64 assembly implements the Fibonacci loop using registers (`eax`, `ecx`, `edx`, `ebx`, `esi`) with a single 8-byte stack adjustment for `rbx` preservation, matching the System V AMD64 ABI. Signed and unsigned comparisons are correctly lowered to handle the `n <= 1` and `n > 46` bounds.

## Verification matrix
- Source semantics and expected result: PASS. The Weave source defines a runtime Fibonacci calculator that reads `WEAVE_AUDIT_N`, defaults to 10 if missing or out-of-range (0..46), and computes `fib(n)` iteratively. The runtime matrix confirms this behavior across 9 cases (e.g., `WEAVE_AUDIT_N=10` yields exit code 55, `WEAVE_AUDIT_N=12` yields 144).
- Weave-to-WIR semantic preservation: PASS. The WIR projection preserves all source statements, including the `fib` function logic, `main`'s getenv/atoi calls, bounds checks, and the iterative while loop.
- WIR-to-raw-LLVM semantic preservation: PASS. Raw LLVM IR declares `getenv` and `atoi`, defines `fib` and `main` with matching control flow, and correctly lowers `le_i32`, `lt_i32`, `gt_i32`, and `add_i32` to `icmp` and `add` instructions.
- Raw LLVM validity, SSA, types, and control flow: PASS. Raw LLVM uses valid SSA form, correctly typed `i32` and `ptr` values, and well-formed basic blocks with proper branching and returns.
- Optimized LLVM semantic preservation: PASS. Optimized LLVM inlines `fib` into `main`, promotes locals to SSA registers, and preserves the `freeze i32 %t3` to prevent undefined behavior from `atoi` propagating. The `icmp ugt i32 %t3.fr, 46` correctly handles the out-of-range check.
- Integer signedness, overflow, shifts, and comparisons: PASS. The `n <= 1` check uses `icmp ult i32 %t3.fr, 2` in optimized LLVM, which is equivalent for non-negative inputs. The `n > 46` check uses `icmp ugt i32 %t3.fr, 46`. The assembly uses `cmpl $0x2e, %eax` followed by `ja` (unsigned above) and `cmpl $2, %eax` followed by `jb` (unsigned below), which is correct. The loop uses `cmpl %ebx, %ecx` with `jle` (signed less-or-equal), which is safe since `n` is bounded to 0..46.
- Calls, return values, ABI, stack alignment, and register use: PASS. The assembly preserves `rbx` via `pushq %rbx` and `popq %rbx`, uses `rdi` for string pointer arguments to `getenv` and `atoi`, and returns the result in `eax`. Stack is 8-byte aligned post-`pushq`, which is valid for these leaf calls in this context.
- Memory safety, lifetime, leaks, and undefined behavior: PASS. No dynamic allocation is used. The `freeze` instruction in optimized LLVM prevents UB from uninitialized or poison values originating from `atoi`. The string literal `WEAVE_AUDIT_N` is properly null-terminated in `.rodata`.
- Target compatibility and native instruction validity: PASS. The target assembly and disassembly use valid x86_64 instructions (`leaq`, `callq`, `movl`, `testq`, `cmpl`, `ja`, `jb`, `addl`, `incl`, `jle`, `retq`) and target `x86_64-pc-linux-gnu`.
- Native runtime cases and expected observable behavior: PASS. The runtime matrix executed the exact linked artifact (SHA `8096a9e8...`) and all 9 cases passed, matching expected exit codes from the sidecar (SHA `55704fe7...`).
- Compiler-generated overhead remaining in final native code: PASS. No avoidable overhead remains. The loop is compact (7 instructions in the inner loop header), uses registers efficiently, and has no unnecessary spills or reloads. The `nopw` is standard alignment padding.

## Blocking findings
None found.

## Non-blocking opportunities
- The `fib` function could be optimized further at the source level using a closed-form formula (Binet's formula) or memoization, but this is a source algorithm choice, not a compiler-generated overhead issue.
- The loop-vectorize and SLP-vectorizer passes missed, which is expected for this sequential recurrence. No action needed.
- The raw LLVM IR contains `alloca` and `load`/`store` traffic, but these are fully optimized away in the optimized LLVM and final assembly, so no overhead remains.

## Suggested verification
- Add a runtime case for `WEAVE_AUDIT_N=46` to verify the upper bound of the Fibonacci sequence (expected `fib(46) = 1836311903`, which fits in `i32`).
- Add a runtime case for `WEAVE_AUDIT_N=2147483648` (or similar large value) to verify the `gt_i32 n 46` fallback works correctly for values exceeding `i32` positive range if `atoi` parses them.
- Verify the `freeze` instruction's necessity by testing with a sanitized `atoi` that returns poison to ensure no UB propagates.
