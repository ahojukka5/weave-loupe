...
- **Audit timestamp (UTC):** `2026-07-27T23:57:39+00:00`
- **Audited source Git SHA:** `91c621242c9d45568048ea8ff717f8d9d4c76685`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `91c621242c9d45568048ea8ff717f8d9d4c76685`
...
- **GitHub run ID:** `30315753299`
- **GitHub workflow SHA:** `cf6ae0b79f7f6d338e6aaddd3ddb72a744b81f88`
...
- `build_manifest` — SHA-256 `7af9d75ee06f8ddc229e7dfb43b5d7172f4e24b9f76ad28333f3358b908ed31d`
...
- `llvm` — SHA-256 `a61f12f463ea01ddf637e0cf3b0cfec3997335d7821575ae4e72caaa4a8e0c63`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-116hC5/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-4p6drn4u/.audit.loupe.9ey5f2gx/artifacts/program",
...
## Summary
The final native program for `docs/audit/fibonacci_runtime.weave` is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The Weave source semantics are preserved through cleaned WIR, raw LLVM, optimized LLVM, target assembly, and the linked executable. The `fib` function is successfully inlined into `main`, and the runtime loop is preserved as required by the runtime-input design. All 9 configured runtime matrix cases pass against the hash-addressed sidecar, affirmatively proving the observable behavior of the exact linked artifact.

## Verification matrix
- Source semantics and expected result: PASS. The Weave source defines a `fib` function that returns `n` for `n <= 1` and iteratively computes Fibonacci otherwise. `main` reads `WEAVE_AUDIT_N`, falls back to 10 if missing, negative, or >46, and returns `fib(n)`. The runtime matrix confirms expected exit codes (e.g., 55 for n=10, 144 for n=12).
- Weave-to-WIR semantic preservation: PASS. The WIR review projection exactly matches the source control flow, comparisons (`le_i32`, `lt_i32`, `gt_i32`), external calls (`getenv`, `atoi`), and variable assignments.
- WIR-to-raw-LLVM semantic preservation: PASS. Raw LLVM IR declares `getenv` and `atoi`, defines `main` and `internal i32 @fib`, and uses matching `icmp` predicates (`slt`, `sgt`, `sle`) and `add i32` operations. Stack allocas correspond to WIR `let` bindings.
- Raw LLVM validity, SSA, types, and control flow: PASS. The raw LLVM is valid SSA with proper branching, block labels, and typed instructions. No `undef` or `poison` uses are reported in the analysis JSON.
- Optimized LLVM semantic preservation: PASS. The optimized LLVM inlines `fib`, uses `freeze i32 %t3` to safely guard `atoi` results, and preserves the loop via `phi` nodes. The bounds check `icmp ugt i32 %t3.fr, 46` correctly handles the fallback.
- Integer signedness, overflow, shifts, and comparisons: PASS. The source limits `n` to 0..46, preventing `i32` overflow in `add i32`. The optimized LLVM uses `ult` and `ugt` for the bounds check after `freeze`, which is semantically equivalent to the signed checks for non-negative values. Native code uses `cmpl $0x2e, %eax` and `ja` correctly.
- Calls, return values, ABI, stack alignment, and register use: PASS. The native code follows the System V AMD64 ABI: `leaq` for string pointer in `%rdi`, `callq getenv@PLT`, `movq %rax, %rdi` for `atoi`, and `ret` with the result in `%eax`. Stack is 16-byte aligned (`pushq %rbx`).
- Memory safety, lifetime, leaks, and undefined behavior: PASS. No dynamic allocation is used. The `freeze` instruction in optimized LLVM prevents UB from uninitialized or poison values. The string literal is safely stored in `.rodata`.
- Target compatibility and native instruction validity: PASS. The target assembly and disassembly use valid x86_64 instructions (`leaq`, `callq`, `testq`, `cmpl`, `jle`, `addl`). The target triple is `x86_64-pc-linux-gnu`.
- Native runtime cases and expected observable behavior: PASS. The analysis JSON shows 9 runtime cases, all passed. The exact executable hash matches the runtime matrix target.
- Compiler-generated overhead remaining in final native code: PASS. The final native code is highly optimized, using only 25 instructions in `main`. The loop is compact (`movl`, `addl`, `incl`, `cmpl`, `jle`) with no unnecessary spills or stack traffic inside the loop.

## Blocking findings
None found.

## Non-blocking opportunities
- The loop in `main` is not vectorized, as noted in the LLVM optimization record. This is a source algorithm limitation (recurrence-based Fibonacci) rather than compiler-generated overhead, and vectorization is not beneficial here.
- The raw LLVM IR uses stack allocas for `n`, `previous`, `current`, and `index`, which are optimized away in the final code. This is standard and non-blocking.

## Suggested verification
- Add a runtime case for `WEAVE_AUDIT_N=46` to explicitly verify the upper boundary of the non-fallback range.
- Add a runtime case for `WEAVE_AUDIT_N=45` to verify a large valid Fibonacci number (1134903170) just below the `i32` overflow threshold.
