...
- **Report content SHA-256:** `3d4f7bff2998c058d644323d519e16ef9bd498f0d8701d3b9ccacc4f5722de7a`
- **Audit timestamp (UTC):** `2026-07-30T05:52:52+00:00`
- **Re-audit no later than (UTC):** `2026-08-29T05:52:52+00:00`
...
- **Audited source Git SHA:** `4d81a425dcf67a651ce38aad2a41464f08fc5ed2`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `4d81a425dcf67a651ce38aad2a41464f08fc5ed2`
- **Auditor content SHA-256:** `7779111e42f6c0ac2e057a5e2942d459954da3202b86497feb58b365ed2ba4c9`
- **weavec Git SHA:** `b5e0c0c248fa8ad3af631cff8f814431385df2b5`
- **weavec binary SHA-256:** `51422f3bf730276673e7846ba5beb2102cb69eca79e07929ce81257d1aeab359`
- **weavec version:** `weavec v0.3.0+git.b5e0c0c248fa`
...
- **LLM max tokens:** `4096`
- **LLM temperature:** `0.0`
- **LLM prompt SHA-256:** `3b3b1be775293d53b9179a2255dd4073f42219d4527c50d1b209ff00d11b63a7`
- **LLM request SHA-256:** `7a48027abcc2ac9c46a815c0a0b12612fe6b2622730695b7ddaeabae5fb732b2`
- **Provider-reported model:** `z-ai/glm-5.2`
- **Provider response ID:** `chatcmpl-a6e50bbc-c2a4-4a75-8d34-dd2c6ef8078c`
- **Provider system fingerprint:** `unavailable`
- **Provider finish reason:** `stop`
- **Provider created (Unix):** `1785390773`
- **Provider prompt tokens:** `19262`
- **Provider completion tokens:** `1051`
- **Provider total tokens:** `20313`
- **GitHub run ID:** `30517822020`
- **GitHub workflow SHA:** `4d81a425dcf67a651ce38aad2a41464f08fc5ed2`
...
- `build_manifest` — SHA-256 `a47674621102b77e1a13f86c4b8c3d1e08755d898b889d5694862e706c9b80b8`
...
- `llvm` — SHA-256 `f6c9991d73c11cfb50310e4fe3ff624959017f0c3bf2b2dba54c92811605aba7`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-NgPrRy/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-cyahdwa2/.audit.loupe.b3iruo53/artifacts/program",
  "sources": [
    "docs/audit/fibonacci_runtime.weave"
  ]
}
...
## Summary
The final native program for `docs/audit/fibonacci_runtime.weave` is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The source semantics define a runtime Fibonacci calculator that reads `WEAVE_AUDIT_N`, clamps the value to `[0, 46]` (defaulting to 10 if missing, negative, or too large), and computes the corresponding Fibonacci number. The Weave-to-WIR projection preserves this logic exactly. Raw LLVM IR implements the logic with valid SSA, explicit stack allocations, and correct signed comparisons. Optimized LLVM IR successfully inlines `fib` into `main`, promotes all stack allocations to registers, and preserves the input-dependent loop as required. The final x86_64 assembly uses correct signed comparisons (`cmpl %ebx, %ecx; jle`), proper System V ABI conventions for `getenv` and `atoi`, and minimal stack manipulation (8-byte alignment via `pushq %rbx`). The configured runtime matrix passes all 9 cases, and the native optimization budget is satisfied with no exceeded limits or missing required structures.

## Verification matrix
- Source semantics and expected result: PASS. The source defines a Fibonacci loop bounded by runtime input clamped to `[0, 46]`. Runtime matrix cases confirm expected results (e.g., `WEAVE_AUDIT_N=10` yields exit code 55, `WEAVE_AUDIT_N=12` yields 144).
- Weave-to-WIR semantic preservation: PASS. The WIR projection matches the source structure, including `getenv`, `atoi`, clamping logic, and the `fib` loop.
- WIR-to-raw-LLVM semantic preservation: PASS. Raw LLVM IR implements the WIR logic with correct `icmp` predicates (`sle`, `slt`, `sgt`, `ne`) and control flow.
- Raw LLVM validity, SSA, types, and control flow: PASS. The IR uses valid SSA, correct types (`i32`, `ptr`), and well-formed basic blocks with explicit `br` and `ret` instructions.
- Optimized LLVM semantic preservation: PASS. Optimized LLVM inlines `fib`, promotes registers, and preserves the input-dependent loop and clamping logic. The `freeze` instruction correctly handles potential `undef` from `atoi`.
- Integer signedness, overflow, shifts, and comparisons: PASS. The loop uses `add i32` which is safe for `n <= 46` (max Fibonacci value 1836311903, within `i32` range). Signed comparisons are used correctly for clamping.
- Calls, return values, ABI, stack alignment, and register use: PASS. `getenv` and `atoi` are called via PLT with correct argument registers (`%rdi`). `main` returns `i32` in `%eax`. Stack is kept 16-byte aligned via `pushq %rbx` (8 bytes) + return address (8 bytes).
- Memory safety, lifetime, leaks, and undefined behavior: PASS. No dynamic allocation is performed. String literal `@.str0` is properly null-terminated. `getenv` return is null-checked before passing to `atoi`.
- Target compatibility and native instruction validity: PASS. The target triple is `x86_64-pc-linux-gnu`. Instructions are valid x86_64 and compatible with the specified AMD EPYC architecture.
- Native runtime cases and expected observable behavior: PASS. All 9 runtime cases pass, matching expected exit codes and empty stdout/stderr.
- Configured native limits, required call targets, and loop backedges: PASS. `main` has 25 instructions (limit 32), 1 backward conditional branch (limit 1), 2 direct calls (limit 2), and required calls to `atoi@plt` and `getenv@plt` are present.
- Compiler-generated overhead remaining in final native code: PASS. The final code is compact, with no unnecessary spills or reloads inside the loop. The loop body uses 7 instructions, which is optimal for this Fibonacci implementation.

## Blocking findings
None found.

## Non-blocking opportunities
- The loop in `main` is not vectorized because the Fibonacci recurrence is inherently sequential. This is a fundamental algorithmic limitation, not compiler overhead.
- The `atoi` function is used for integer parsing, which is a C standard library function with undefined behavior on some invalid inputs. However, the `freeze` instruction in optimized LLVM and the runtime matrix case `non-numeric-atoi-zero` confirm that the program handles this safely in practice.

## Suggested verification
- Add a runtime case for `WEAVE_AUDIT_N=46` to verify the upper bound of the Fibonacci calculation (expected exit code 1836311903 mod 256, since exit codes are 8-bit).
- Add a runtime case for `WEAVE_AUDIT_N=2147483648` (overflow for 32-bit signed integer) to verify `atoi` behavior on extreme values, though this is outside the current clamping range.
