...
- **Audit timestamp (UTC):** `2026-07-27T21:30:56+00:00`
- **Audited source Git SHA:** `638983b38d66f13f7a9c21fc252f5c8860ab9525`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `638983b38d66f13f7a9c21fc252f5c8860ab9525`
...
- **GitHub run ID:** `30307056695`
- **GitHub workflow SHA:** `606b87639a1f03fb2fc5658a4e76a58723400723`
...
- `build_manifest` — SHA-256 `583ba041d5e199fbae2d92bcf379d5ec3b7633ca5825f905d95ccf33701c1850`
...
- `llvm` — SHA-256 `764d9a7b2e7191443455fd380132db2a7379f629397ae170e4b60913f55fbd2d`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-A7qObx/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-frywh63b/.audit.loupe.utvlgua9/artifacts/program",
...
## Summary
The final native program correctly implements the runtime-input Fibonacci corpus. The Weave source reads `WEAVE_AUDIT_N`, clamps the parsed value to `[0, 46]` with a fallback of `10`, and computes `fib(n)` iteratively. The cleaned WIR preserves that semantics exactly. Raw LLVM lowers `fib` and `main` to valid SSA with correct signed `i32` comparisons and arithmetic. The optimized LLVM inlines `fib`, promotes locals to SSA, and preserves the loop because `n` is runtime-dependent. Target assembly and linked disassembly show a valid x86-64 `main` that calls `getenv`/`atoi` through PLT, clamps with unsigned comparisons against 46, handles `n < 2` as an early return, and executes a compact Fibonacci loop using `%eax`, `%edx`, `%ecx`, and `%ebx` with a signed `jle` loop exit. ABI, stack alignment, callee-saved register preservation, and return convention are correct. No merge-blocking defect is present.

## Verification matrix
- Source semantics and expected result: PASS. Weave source defines `fib(n)` as `n` for `n <= 1`, otherwise iteratively computes Fibonacci with `previous=0`, `current=1`, `index=2..n`; `main` reads `WEAVE_AUDIT_N`, falls back to `10` when missing/negative/greater than 46, and returns `fib(n)`.
- Weave-to-WIR semantic preservation: PASS. WIR projection matches source: `le_i32 n 1` guard, `while (le_i32 index n)`, `add_i32 previous current`, `set previous current`, `set current next`, `set index (add_i32 index 1)`, and `main` getenv/atoi/clamp sequence.
- WIR-to-raw-LLVM semantic preservation: PASS. Raw LLVM `@fib` uses `icmp sle i32 %n, 1`, `previous.addr/current.addr/index.addr` allocas, `add i32` for `next` and `index+1`; `@main` uses `icmp ne ptr %t1, null`, `icmp slt i32 %t4, 0`, `icmp sgt i32 %t6, 46`, and `call i32 @fib(i32 %t8)`.
- Raw LLVM validity, SSA, types, and control flow: PASS. SSA is valid, types are `i32`/`ptr`, basic blocks are terminated with `br`/`ret`, `alloca`/`load`/`store` are consistent, no `undef`/`poison` uses reported.
- Optimized LLVM semantic preservation: PASS. `fib` is inlined into `main`; `%t3.fr = freeze i32 %t3`; clamp uses `icmp ugt i32 %t3.fr, 46`; early return uses `icmp ult i32 %t3.fr, 2`; loop PHI nodes initialize `index=2`, `current=1`, `previous=0` and update as `current.addr.05.i`, `previous.addr.04.i`, `%t5.i = add`, `%t8.i = add ... 1`; exit condition is `icmp sgt i32 %t8.i, %0`.
- Integer signedness, overflow, shifts, and comparisons: PASS. Source clamps to `0..46`, so `fib` loop arithmetic cannot overflow `i32`; optimized LLVM uses `ult`/`ugt` for clamp because values are nonnegative after the `slt` check, and `sgt` for the loop exit preserving signed semantics. Native code uses `cmpl $46, %eax; ja` and `cmpl $2, %eax; jb` for clamp, `cmpl %ebx, %ecx; jle` for loop exit.
- Calls, return values, ABI, stack alignment, and register use: PASS. `getenv`/`atoi` are declared with C-compatible signatures; native code passes `rdi` for pointer argument, receives `rax`/`eax`, preserves `rbx` with `pushq %rbx`/`popq %rbx`, returns in `eax`, and uses 8 bytes of stack with 16-byte alignment maintained.
- Memory safety, lifetime, leaks, and undefined behavior: PASS. No allocations or frees; string literal is a private unnamed_addr constant; `getenv` result is null-checked before `atoi`; `atoi` result is frozen before use; no out-of-bounds access or UB evident.
- Target compatibility and native instruction validity: PASS. Target triple is `x86_64-pc-linux-gnu`; instructions are valid x86-64; PLT/GOT linkage for glibc `getenv`/`atoi` is standard; `.note.GNU-stack` present; `_start` calls `main` through libc.
- Compiler-generated overhead remaining in final native code: PASS. No avoidable overhead remains: `fib` is inlined, allocas eliminated, loop is compact with register-only state; remaining instructions are essential for runtime input, clamp, and Fibonacci iteration.

## Blocking findings
None found.

## Non-blocking opportunities
- The Fibonacci loop is not vectorized, as recorded by LLVM optimization remarks. This is a source algorithm/loop-carried dependency limitation, not compiler-generated overhead, and is non-blocking.
- `atoi` is not a safe input parser for untrusted environment variables, but the audit harness supplies controlled `0..46` input and the source explicitly clamps out-of-range values to 10, so this is a source-level robustness note only.
- Raw LLVM uses stack allocas for `fib` locals and `main`’s `n`, but optimized LLVM and native code remove them, so this is not a defect.

## Suggested verification
- Run the executable with `WEAVE_AUDIT_N` set to boundary values `0, 1, 2, 45, 46` and compare against expected Fibonacci numbers.
- Run with missing `WEAVE_AUDIT_N`, negative values, and values greater than 46 to confirm fallback to `fib(10)`.
- Confirm `main` exit status matches the returned `fib(n)`.
- Optionally inspect GOT/PLT relocations to verify `getenv`/`atoi` resolve to glibc symbols.
