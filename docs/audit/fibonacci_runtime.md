...
- **Report content SHA-256:** `e06583b33942d2461f584e04015ecd7e1b380c2a97e7d4399b2e4f403a3b7745`
- **Audit timestamp (UTC):** `2026-07-28T08:00:43+00:00`
- **Re-audit no later than (UTC):** `2026-08-27T08:00:43+00:00`
...
- **Audited source Git SHA:** `5b939878fe34bef22eb5f55db480953498bc5c59`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `5b939878fe34bef22eb5f55db480953498bc5c59`
- **Auditor content SHA-256:** `2ef87d18e467e588385e701a6c1ea269b098109f9a699941913ae983b43efcc5`
...
- **GitHub run ID:** `30340528225`
- **GitHub workflow SHA:** `b0aa39f4f243a1e9e08775a3f74dc744dd040055`
...
- `build_manifest` — SHA-256 `978fe15ca5f4d7a2a54955a197d54854a4a71964d0438e42f233dfd805d7b721`
...
- `llvm` — SHA-256 `dfc734e846a265025243f2e34578e9a6266be74205fb09649500e43f4ab3f684`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-0YUWSp/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-1ywz9pqd/.audit.loupe.iyvt_vb9/artifacts/program",
...
## Summary
The final native program for `docs/audit/fibonacci_runtime.weave` is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The source defines a runtime-driven Fibonacci computation with input clamping, which is preserved cleanly through the WIR and raw LLVM. The optimizer successfully inlines `fib` into `main`, promotes stack locals to SSA registers, and folds the input range checks into a single unsigned comparison. The final x86_64 assembly uses a compact register-allocated loop that matches the source semantics exactly. All 9 configured runtime matrix cases pass against the hash-addressed sidecar, confirming end-to-end behavioral correctness.

## Verification matrix
- Source semantics and expected result: PASS. The Weave source defines `fib(n)` returning `n` for `n <= 1`, otherwise iterating to compute Fibonacci. `main` reads `WEAVE_AUDIT_N`, defaults to 10 if missing, clamps negative or `>46` values to 10, and returns `fib(n)`. Runtime cases confirm these semantics.
- Weave-to-WIR semantic preservation: PASS. The WIR projection preserves the `getenv`/`atoi` externs, the `fib` control flow and loop, and `main`'s input reading and clamping logic exactly.
- WIR-to-raw-LLVM semantic preservation: PASS. Raw LLVM implements the WIR directly with `alloca`/`store`/`load` for locals, `icmp sle` for `le_i32`, `icmp slt` for `lt_i32`, and `icmp sgt` for `gt_i32`. Control flow edges match the WIR.
- Raw LLVM validity, SSA, types, and control flow: PASS. The raw LLVM is well-formed, uses valid SSA values, and has no `undef` or `poison` uses (confirmed by analysis JSON: `poison_uses: 0`, `undef_uses: 0`).
- Optimized LLVM semantic preservation: PASS. The optimized LLVM inlines `fib`, promotes all locals to SSA `phi` nodes, and combines the `<0` and `>46` checks into a single `icmp ugt i32 %t3.fr, 46` (valid because `freeze` is used on the `atoi` result). The loop logic is mathematically identical to the source.
- Integer signedness, overflow, shifts, and comparisons: PASS. The `ult i32 %t3.fr, 2` check correctly handles the `n <= 1` base case. The loop uses `add i32` which is safe for `n <= 46` (max value 1,836,311,903, well within `i32` range). The `cmpl $46, %eax` followed by `ja` correctly implements the unsigned range check.
- Calls, return values, ABI, stack alignment, and register use: PASS. `getenv` and `atoi` are called via PLT with arguments in `%rdi` per System V ABI. `main` aligns the stack via `pushq %rbx` (8 bytes) on top of the 8-byte return address, maintaining 16-byte alignment. `%rbx` is callee-saved and restored before `retq`.
- Memory safety, lifetime, leaks, and undefined behavior: PASS. The program uses only stack locals and read-only string literals. No heap allocation occurs. The `freeze` instruction in optimized LLVM prevents UB from `atoi` returning an uninitialized value.
- Target compatibility and native instruction validity: PASS. The target triple is `x86_64-pc-linux-gnu`. The assembly uses standard x86_64 instructions (`leaq`, `callq`, `testq`, `cmpl`, `addl`, `jle`, etc.) and links correctly against glibc 2.39.
- Native runtime cases and expected observable behavior: PASS. All 9 runtime cases pass. The executable hash matches the analysis target. Cases cover missing input, 0, 1, 2, 10, 12, -1, 47, and "abc", all producing the expected exit codes.
- Compiler-generated overhead remaining in final native code: PASS. The final assembly is 25 instructions. The loop is 7 instructions with no unnecessary spills or reloads. The only overhead noted in the optimization record (virtual register copies) is resolved during register allocation.

## Blocking findings
None found.

## Non-blocking opportunities
- The loop was not vectorized (noted in the optimization record). This is expected because the Fibonacci recurrence has a loop-carried dependency (`next = previous + current`), making it non-vectorizable without advanced rewriting. This is a source algorithm characteristic, not compiler overhead.
- The raw LLVM uses `alloca`/`store`/`load` for all locals, which is standard for unoptimized IR and fully eliminated by LLVM's `mem2reg` pass. This is not a defect.

## Suggested verification
- Add a runtime case for `WEAVE_AUDIT_N=46` to explicitly test the upper boundary of the non-clamped path (expected exit code 1836311903).
- Add a runtime case for `WEAVE_AUDIT_N=45` to verify the loop iterates correctly near the boundary (expected exit code 1134903170).
- Add a runtime case for a large non-numeric string like `WEAVE_AUDIT_N=99999999999999999999` to verify `atoi` overflow behavior does not cause unexpected crashes or exit codes.
