...
- **Audit timestamp (UTC):** `2026-07-28T04:22:57+00:00`
- **Re-audit no later than (UTC):** `2026-08-27T04:22:57+00:00`
...
- **Audited source Git SHA:** `e18b8320bf486fc986413dee0780d36abb300556`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `e18b8320bf486fc986413dee0780d36abb300556`
...
- **GitHub run ID:** `30328531893`
- **GitHub workflow SHA:** `6d8aa5198a1976042ca3e05b48e1c482010d803d`
...
- `build_manifest` — SHA-256 `47f641eca251c3d1454933ac099690a7a77e70012473d26c8f73df8b5b60391e`
...
- `llvm` — SHA-256 `d50da173db5a8b8424ac910e51b1b6448239ee4c06a9a17707cad94367131f20`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-B9aDGv/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-bkvbs2lo/.audit.loupe.r55u4rkh/artifacts/program",
...
## Summary
The final native program is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The Weave source computes an input-dependent Fibonacci value bounded to 0..46, and the optimized LLVM preserves this loop without constant-folding it. The final x86_64 assembly correctly implements the System V ABI, uses signed comparisons matching the source semantics, and executes cleanly across all 9 runtime matrix cases.

## Verification matrix
- Source semantics and expected result: PASS. The Weave source defines `fib` to return `n` for `n <= 1`, otherwise iteratively compute Fibonacci up to `n`. `main` reads `WEAVE_AUDIT_N`, defaults to 10 if missing, clamps negative or >46 values to 10, and returns `fib(n)`.
- Weave-to-WIR semantic preservation: PASS. The WIR projection preserves all control flow, comparisons (`le_i32`, `lt_i32`, `gt_i32`, `ne_ptr`), calls, and variable assignments exactly as in the source.
- WIR-to-raw-LLVM semantic preservation: PASS. Raw LLVM uses `icmp sle`, `icmp slt`, `icmp sgt`, and `icmp ne` matching the WIR. The `fib` loop structure, `getenv`/`atoi` calls, and clamping logic are preserved.
- Raw LLVM validity, SSA, types, and control flow: PASS. The raw LLVM is well-formed, uses valid SSA values, correct types (`i32`, `ptr`), and proper branching. No `undef` or `poison` uses are reported.
- Optimized LLVM semantic preservation: PASS. The optimized LLVM inlines `fib` into `main`, preserves the loop via `while.body1.i`, and uses `freeze i32 %t3` to safely handle `atoi`'s potential undefined behavior. The loop exit condition `icmp sgt i32 %t8.i, %0` correctly mirrors `le_i32 index n`.
- Integer signedness, overflow, shifts, and comparisons: PASS. All comparisons are signed (`slt`, `sgt`, `sle`) matching the source. The `cmpl $46, %eax; ja` in native code is a valid unsigned comparison for the upper bound check since negative values are handled by the prior `slt` check. The `cmpl $2, %eax; jb` correctly handles the `n <= 1` base case.
- Calls, return values, ABI, stack alignment, and register use: PASS. `getenv` and `atoi` are called via PLT with arguments in `rdi` and results in `rax`. `main` saves/restores `rbx` and returns `i32` in `eax`. Stack is 16-byte aligned.
- Memory safety, lifetime, leaks, and undefined behavior: PASS. The optimized LLVM inserts a `freeze` on `atoi`'s return value, preventing UB propagation. No memory leaks or unsafe accesses are present.
- Target compatibility and native instruction validity: PASS. The target triple is `x86_64-pc-linux-gnu`. The disassembly shows valid x86_64 instructions, correct PLT stubs, and standard CRT initialization.
- Native runtime cases and expected observable behavior: PASS. All 9 runtime cases passed, including default, boundary, and invalid inputs. The executable SHA256 matches the runtime matrix target.
- Compiler-generated overhead remaining in final native code: PASS. The final native code is compact (25 instructions in `main`), with no redundant stack traffic or unnecessary moves in the hot loop. The loop uses a minimal register dance (`eax`, `edx`, `esi`) to implement the two-variable Fibonacci recurrence.

## Blocking findings
None found.

## Non-blocking opportunities
- The loop-vectorizer and SLP-vectorizer missed opportunities are expected and non-blocking, as the Fibonacci recurrence is inherently sequential.
- The `freeze` instruction in optimized LLVM is a necessary safety measure for `atoi`'s UB, not an overhead defect.

## Suggested verification
- Add a runtime case for `WEAVE_AUDIT_N=46` to verify the upper boundary of the Fibonacci computation.
- Add a runtime case for `WEAVE_AUDIT_N=45` to verify a large valid Fibonacci number (1134903170, which fits in 32-bit signed int).
