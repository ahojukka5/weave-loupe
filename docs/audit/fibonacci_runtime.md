...
- **Report content SHA-256:** `79f6a4f7c44f1a1ca0e415ff9f6b7982e4493dea4059c2fa2a618d5332b7bfe9`
- **Audit timestamp (UTC):** `2026-07-31T23:55:18+00:00`
- **Re-audit no later than (UTC):** `2026-08-30T23:55:18+00:00`
...
- **Audited source Git SHA:** `8fdce449c265fbc5a18b3a6a83c708db5871cdde`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `8fdce449c265fbc5a18b3a6a83c708db5871cdde`
- **Auditor content SHA-256:** `9ab9deb9f761ea579fe7c3e6995dd18eb18d13a60382a24e58b94d26f9119e2f`
...
- **LLM max tokens:** `4096`
- **LLM temperature:** `0.0`
- **LLM prompt SHA-256:** `eff27976750b3c2b1f1b1dff6e5844522718698e9b4ab11bfe4d1c286d93bc87`
- **LLM request SHA-256:** `19088777b6012bb0ab873836d3a82677f43417522517fdf13d7c9c61b27bc8e2`
- **Provider-reported model:** `z-ai/glm-5.2`
- **Provider response ID:** `chatcmpl-ca2de57c-7a88-4c74-9aa5-4f15b82faaae`
- **Provider system fingerprint:** `unavailable`
- **Provider finish reason:** `stop`
- **Provider created (Unix):** `1785542119`
- **Provider prompt tokens:** `22072`
- **Provider completion tokens:** `1260`
- **Provider total tokens:** `23332`
- **GitHub run ID:** `30674124208`
- **GitHub workflow SHA:** `d1c068e3a2585c80a04369c44c85fd66ac678810`
...
- `build_manifest` — SHA-256 `799815acb524db739e1cad04ed4d19b141046f2fbbabcd1257c3998bd78ddc7a`
...
- `llvm` — SHA-256 `b91af294cc3e7e0ce61a07585c3f85dbce69410d62d9f3906bc470d28b3834ef`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-wqnmBG/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-sudrd5m5/.audit.loupe.eokq37ms/artifacts/program",
  "sources": [
...
## Summary
The final native program is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The Weave source computes a Fibonacci number based on the `WEAVE_AUDIT_N` environment variable, clamping out-of-range values to 10. The cleaned WIR, raw LLVM, optimized LLVM, target assembly, and linked executable disassembly all preserve this semantics. The optimized LLVM inlines the `fib` function and promotes stack allocations to SSA registers, eliminating all `alloca`, `load`, and `store` instructions. The final native code consists of a compact loop in `main` using `eax`, `ecx`, and `edx` for the Fibonacci calculation, correctly returning the result in `eax` per the x86-64 System V ABI. All 9 configured runtime cases passed, matching expected exit codes. The optimized LLVM contract and native optimization budget are both satisfied.

## Verification matrix
- Source semantics and expected result: PASS. The Weave source defines a `fib` function and a `main` function that reads `WEAVE_AUDIT_N`, clamps it to 0..46, and returns `fib(n)`. The runtime matrix confirms expected exit codes (e.g., 55 for n=10, 144 for n=12).
- Weave-to-WIR semantic preservation: PASS. The WIR projection matches the source structure, including `getenv`, `atoi`, `fib`, and `main` with correct control flow and operations.
- WIR-to-raw-LLVM semantic preservation: PASS. The raw LLVM IR implements the WIR semantics, using `alloca` for local variables and correct `icmp` predicates (`sle`, `slt`, `sgt`, `ne`).
- Raw LLVM validity, SSA, and control flow: PASS. The raw LLVM uses valid SSA form, correct types (`i32`, `ptr`), and well-formed basic blocks with explicit `br` and `ret` instructions.
- Optimized LLVM semantic preservation: PASS. The optimized LLVM inlines `fib` into `main`, promotes stack variables to SSA registers, and preserves the loop logic with correct `phi` nodes and `icmp` predicates.
- Configured optimized LLVM metrics, functions, calls, and memory traffic: PASS. The analysis JSON shows the optimized LLVM contract passed with 20 instructions, 1 function (`main`), 2 calls (`atoi`, `getenv`), 0 `alloca`/`load`/`store`, and no poison/undef uses.
- Integer signedness, overflow, shifts, and comparisons: PASS. The source clamps `n` to 0..46, preventing `i32` overflow in `fib` (max Fibonacci value is 1836311903 for n=46). The optimized LLVM uses `ult` for the `< 2` check and `ugt` for the `> 46` check, which is safe since values are non-negative. The loop uses `sgt` for the index comparison, matching the signed `i32` semantics.
- Calls, return values, ABI, stack alignment, and register use: PASS. The native code uses `rdi` for `getenv` and `atoi` arguments, `rax` for return values, and `rbx` as a callee-saved register, aligning with the x86-64 System V ABI. The stack is 16-byte aligned after `pushq %rbx`.
- Memory safety, lifetime, leaks, and undefined behavior: PASS. The optimized LLVM has no `load` or `store` instructions, eliminating stack memory traffic. The `getenv` and `atoi` calls use `nonnull` and `nocapture` attributes, ensuring no memory leaks or undefined behavior.
- Target compatibility and native instruction validity: PASS. The target assembly and disassembly use valid x86-64 instructions (`leaq`, `callq`, `testq`, `cmpl`, `addl`, `incl`, `jle`, `jb`, `ja`, `retq`) for the `x86_64-pc-linux-gnu` target.
- Native runtime cases and expected observable behavior: PASS. All 9 runtime cases passed, including missing input, zero, one, two, ten, twelve, negative, too-large, and non-numeric inputs, with correct exit codes.
- Configured native limits, required call targets, and loop backedges: PASS. The native budget passed with 25 instructions, 2 direct calls (`atoi@plt`, `getenv@plt`), 1 backward conditional branch, and 1 program-owned function (`main`).
- Compiler-generated overhead remaining in optimized LLVM and final native code: PASS. The optimized LLVM is compact with no identity adds or redundant instructions. The native code uses a tight loop with minimal register moves and no stack spills within the loop.

## Blocking findings
None found.

## Non-blocking opportunities
- The raw LLVM uses `alloca` for local variables (`n.addr`, `previous.addr`, `current.addr`, `index.addr`), resulting in stack traffic. However, this is optimized away in the optimized LLVM and native code, so it is not a defect.
- The optimization record notes missed vectorization opportunities for the Fibonacci loop. This is a source algorithm limitation (recurrence-based), not compiler-generated overhead, and vectorization would not change semantics.
- The native code uses `incl %ecx` for the loop index, which is efficient but could theoretically be combined with the `addl` in a more complex macro-op fusion scenario. This is negligible and below the native optimization budget ceiling.

## Suggested verification
- Add a runtime case for `WEAVE_AUDIT_N=46` to verify the maximum Fibonacci value (1836311903) and ensure no `i32` overflow.
- Add a runtime case for `WEAVE_AUDIT_N=45` to check boundary behavior near the clamp limit.
- Include a test with a large numeric string (e.g., `WEAVE_AUDIT_N=999999999999`) to verify `atoi` behavior and clamping logic.
