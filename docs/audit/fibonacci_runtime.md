...
- **Audit timestamp (UTC):** `2026-07-28T03:23:03+00:00`
- **Re-audit no later than (UTC):** `2026-08-27T03:23:03+00:00`
...
- **Audited source Git SHA:** `15dcae1163062f1442fe79dbcc481312bb60425b`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `15dcae1163062f1442fe79dbcc481312bb60425b`
...
- **GitHub run ID:** `30325690683`
- **GitHub workflow SHA:** `f9c14703afca88e256355277bdefb7ecfe977513`
...
- `build_manifest` — SHA-256 `6ebd0f27c599fde7be7fb6c00026482f271fb371199bf40fa06bd3556efb05e1`
...
- `llvm` — SHA-256 `8510a54c099dc1b38f10753cd5f0b37d84dba2802c3dfbe7637d32fb4210164a`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-wTRTq5/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-4koocn55/.audit.loupe.3d3sobzs/artifacts/program",
...
## Summary
The final native program is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The Weave source defines a runtime-input Fibonacci program that reads `WEAVE_AUDIT_N`, clamps the value to `0..46` with a fallback of 10, and computes Fibonacci iteratively. The cleaned WIR preserves this structure exactly. Raw LLVM lowers it to valid SSA with proper signed comparisons and memory operations. Optimized LLVM inlines `fib` into `main`, promotes all stack slots to registers, and preserves the input-dependent loop as required. Target assembly and linked disassembly show a valid x86_64 System V ABI implementation with a compact loop. The runtime matrix of 9 cases passes completely, confirming the exact linked artifact behaves correctly across default, boundary, negative, too-large, and non-numeric inputs. No merge-blocking defects are present.

## Verification matrix
- Source semantics and expected result: PASS. Weave source defines `fib` with base case `n <= 1` returning `n`, otherwise iterating from index 2 to `n` inclusive. `main` reads `WEAVE_AUDIT_N`, defaults to 10, clamps negative or >46 to 10, and returns `fib(n)`. Runtime matrix expected values match (e.g., n=10 -> 55, n=12 -> 144, n=0 -> 0, n=1 -> 1, n=2 -> 1).
- Weave-to-WIR semantic preservation: PASS. WIR projection preserves `getenv`/`atoi` externs, `fib` base case and loop, `main` input handling, clamping, and return call exactly.
- WIR-to-raw-LLVM semantic preservation: PASS. Raw LLVM implements `fib` with `icmp sle i32 %n, 1`, loop with `icmp sle i32 %t1, %n`, and `main` with `icmp ne ptr`, `icmp slt i32`, and `icmp sgt i32` checks matching WIR.
- Raw LLVM validity, SSA, and control flow: PASS. SSA is valid with distinct temporaries, dominator-consistent branches, and no `undef`/`poison` uses. Analysis JSON confirms 0 `undef_uses`, 0 `poison_uses`.
- Optimized LLVM semantic preservation: PASS. `fib` is inlined; stack slots eliminated; loop preserved with `phi` nodes for `index`, `current`, `previous`. Signed and unsigned comparisons correctly handle clamping and base cases.
- Integer signedness, overflow, shifts, and comparisons: PASS. `cmpl $46, %eax; ja` uses unsigned comparison for the >46 clamp, which is safe because `atoi` returns `i32` and the optimized LLVM uses `icmp ugt i32 %t3.fr, 46`. `cmpl $2, %eax; jb` uses unsigned comparison for the `n < 2` base case, matching `icmp ult`. Loop uses `cmpl %ebx, %ecx; jle` (signed) matching `icmp sgt` in optimized LLVM. No overflow risk for n <= 46.
- Calls, return values, ABI, stack alignment, and register use: PASS. `getenv` and `atoi` called via PLT with `%rdi` argument. `main` saves/restores `%rbx`, returns in `%eax`. Stack is 16-byte aligned after `pushq %rbx` (8 bytes + 8 from caller return address = 16). Analysis JSON confirms 8 stack bytes, alignment 16.
- Memory safety, lifetime, leaks, and undefined behavior: PASS. No dynamic allocation. Stack slots in raw LLVM are properly allocated and accessed. Optimized LLVM eliminates all allocas. No null dereference (null check before `atoi`). `freeze i32 %t3` prevents poison propagation.
- Target compatibility and native instruction validity: PASS. Target triple is `x86_64-pc-linux-gnu`. Instructions are valid x86_64. `.note.GNU-stack` section present. PLT entries for glibc functions are standard.
- Native runtime cases and expected observable behavior: PASS. All 9 runtime cases passed. Executable SHA256 matches between metadata and runtime matrix. Sidecar SHA256 matches. Cases cover missing input, 0, 1, 2, 10, 12, -1, 47, and "abc".
- Compiler-generated overhead remaining in final native code: PASS. No avoidable overhead. `fib` is inlined. Stack traffic eliminated. Loop is compact (6 instructions in body). `nopw` padding is standard alignment, not semantic overhead. PLT calls are necessary for external functions.

## Blocking findings
None found.

## Non-blocking opportunities
- The loop could potentially be optimized further with a closed-form formula or matrix exponentiation, but this is a source algorithm choice, not compiler overhead, and is intentionally preserved per the source comment.
- The `freeze i32 %t3.fr` in optimized LLVM is a conservative poison-blocking measure; it is not overhead in final native code.
- Loop vectorization was not possible due to the sequential dependency in Fibonacci; this is inherent to the algorithm, not a compiler defect.

## Suggested verification
- Test with `WEAVE_AUDIT_N=46` to verify the upper boundary of the non-clamped range produces the correct Fibonacci value (1836311903).
- Test with `WEAVE_AUDIT_N=3` to verify the loop executes exactly once and returns 2.
- Verify stack alignment at `callq getenv@PLT` by inspecting `%rsp` value (should be 0x...0).
- Confirm no uninitialized register use by tracing `%ebx` value from entry through all paths to the loop.
