...
- **Audit timestamp (UTC):** `2026-07-28T04:28:21+00:00`
- **Re-audit no later than (UTC):** `2026-08-27T04:28:21+00:00`
...
- **Audited source Git SHA:** `dd37c856d270170cdc4ee2f8db18accfe757b18d`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `dd37c856d270170cdc4ee2f8db18accfe757b18d`
- **Auditor content SHA-256:** `99d9d7f7ded8b919419e525908de7e72cbe0dac0055c3751a97d2445d0781fd6`
...
- **GitHub run ID:** `30328744550`
- **GitHub workflow SHA:** `bd0788510100879bb5a490f2a6006d4564461f95`
...
- `build_manifest` — SHA-256 `8c0413e1b112e616e3a151023288dc2ec7b4565ad3d47cf89b5cbe67d322dff9`
...
- `llvm` — SHA-256 `a5cc710eb702b7db0e608cae35fdd8d47e7ad75e064d190c39f6e61ae87c460a`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-YZEVDF/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-s1jbecik/.audit.loupe.yscwvd2j/artifacts/program",
...
## Summary
The final native program for `fibonacci_runtime.weave` is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The Weave source semantics are cleanly preserved through the WIR and raw LLVM IR. LLVM correctly inlines the `fib` function and promotes its stack-allocated local variables into registers in the optimized LLVM IR. The final x86_64 assembly implements the Fibonacci loop efficiently using registers and correctly handles the `main` function's ABI (preserving callee-saved `rbx` and aligning the stack via `pushq %rbx`). All 9 configured runtime matrix cases pass, confirming that the expected source behavior is faithfully executed by the exact linked artifact.

## Verification matrix
- Source semantics and expected result: PASS. The source defines a `fib` function that returns `n` for `n <= 1` and iteratively computes Fibonacci otherwise. `main` reads `WEAVE_AUDIT_N`, defaults to 10 if missing, negative, or > 46, and returns `fib(n)`. The runtime matrix confirms these exact semantics.
- Weave-to-WIR semantic preservation: PASS. The WIR projection preserves all control flow, variable declarations, and operations from the source, including the `fib` loop and `main`'s input validation.
- WIR-to-raw-LLVM semantic preservation: PASS. The raw LLVM IR explicitly allocates stack slots for variables, uses correct `icmp` predicates (`sle`, `slt`, `sgt`, `ne`), and accurately translates the while loop and if conditions.
- Raw LLVM validity, SSA, types, and control flow: PASS. The raw LLVM IR is valid, uses proper SSA form for temporaries, and has well-formed basic blocks and branching.
- Optimized LLVM semantic preservation: PASS. The optimized LLVM IR inlines `fib` into `main`, promotes stack slots to SSA registers, and uses a `freeze` instruction on the `atoi` result to prevent undefined behavior from propagating, preserving the exact source semantics.
- Integer signedness, overflow, shifts, and comparisons: PASS. The optimized LLVM uses `icmp ult` for the combined upper bound check (`> 46`), which is safe because negative values wrap to large unsigned values and correctly trigger the fallback. The `fib` loop uses `icmp sgt` for the loop exit condition, matching the signed `le_i32` source condition.
- Calls, return values, ABI, stack alignment, and register use: PASS. The native code uses `rdi` for the `getenv` and `atoi` arguments, handles return values in `rax`/`eax`, and preserves the callee-saved `rbx` register. The `pushq %rbx` instruction correctly aligns the stack to 16 bytes.
- Memory safety, lifetime, leaks, and undefined behavior: PASS. The program relies on standard library functions and uses a read-only string literal. The `freeze` instruction in optimized LLVM prevents UB from unproven `atoi` returns. No memory leaks or unsafe accesses are present.
- Target compatibility and native instruction validity: PASS. The linked executable disassembly shows valid x86_64 instructions, correct PLT setup for external calls, and standard ELF initialization routines.
- Native runtime cases and expected observable behavior: PASS. All 9 runtime cases in the analysis JSON passed, matching expected exit codes for default, boundary, valid, and invalid inputs.
- Compiler-generated overhead remaining in final native code: PASS. The final native code is highly optimized, using a tight register-only loop for Fibonacci computation. No unnecessary stack traffic or redundant instructions remain in the final `main` function.

## Blocking findings
None found.

## Non-blocking opportunities
- The loop vectorizer missed vectorizing the Fibonacci loop due to the loop-carried dependency. This is an inherent algorithmic limitation of the iterative Fibonacci sequence, not compiler-generated overhead.
- The raw LLVM IR uses `alloca` and `store`/`load` for local variables, which is standard unoptimized IR. These are completely eliminated by LLVM's mem2reg pass and do not appear in the final native code.

## Suggested verification
- Add a runtime case for `WEAVE_AUDIT_N=46` to explicitly verify the exact upper boundary of the allowed range without triggering the fallback.
- Add a runtime case for a large non-numeric string (e.g., `WEAVE_AUDIT_N=999999999999999999`) to ensure `atoi` overflow behavior consistently falls back to the default as expected.
