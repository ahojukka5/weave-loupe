...
- **Report content SHA-256:** `dff5170557f131ba057fd3c8f7063b1b1d56eea1cda2973bd62886b4cbbeb59b`
- **Audit timestamp (UTC):** `2026-07-31T16:31:45+00:00`
- **Re-audit no later than (UTC):** `2026-08-30T16:31:45+00:00`
...
- **Audited source Git SHA:** `cb6ffbf8e85a267e35e6d1df20abec20375cbcf6`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `cb6ffbf8e85a267e35e6d1df20abec20375cbcf6`
- **Auditor content SHA-256:** `20ec07c366f56ca27e2d39cb42dd0cbf394f791f587eac2595f3b69fc00395ec`
...
- **LLM max tokens:** `4096`
- **LLM temperature:** `0.0`
- **LLM prompt SHA-256:** `0ac3b34989108b469b43cef30d032bbcffae6c0808ce2c309d34fd8783000cf0`
- **LLM request SHA-256:** `231cac6ff838c59883d81f7779af5bb529104a63b1b26765e0b40d42fa46841e`
- **Provider-reported model:** `z-ai/glm-5.2`
- **Provider response ID:** `chatcmpl-b73c040b-77fc-4344-ab5f-a1e043c36344`
- **Provider system fingerprint:** `unavailable`
- **Provider finish reason:** `stop`
- **Provider created (Unix):** `1785515506`
- **Provider prompt tokens:** `20140`
- **Provider completion tokens:** `1485`
- **Provider total tokens:** `21625`
- **GitHub run ID:** `30647408621`
- **GitHub workflow SHA:** `b05510c0473ac86b2cfee2d451999c49a830d60a`
...
- `build_manifest` — SHA-256 `73230d9d293ca1e64840e74400fcfe7e234dc21454c70766b11192b89be488da`
...
- `llvm` — SHA-256 `fa2a9022a5a6ed2fee2e54c9be1a2943014e150663bcb3875a251b6c9d1883b1`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-iQL4o6/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-ag6nzt5b/.audit.loupe.lrscl5es/artifacts/program",
  "sources": [
    "docs/audit/fibonacci_runtime.weave"
  ]
}
...
## Summary
The final native program for `fibonacci-runtime` is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The Weave source computes a Fibonacci sequence for an environment-provided integer `n` (defaulting to 10 if missing, negative, or greater than 46) and returns the result as the process exit code. The WIR accurately projects these semantics, and the raw LLVM IR preserves them through straightforward memory operations and control flow. LLVM optimizes the program by inlining `fib` into `main`, promoting stack slots to SSA registers, and simplifying the input bounds checks. The optimized LLVM IR is valid, free of undefined behavior and poison, and satisfies all configured structural metrics. The final x86_64 assembly correctly implements the optimized IR, adheres to the System V ABI, and executes the Fibonacci loop efficiently. All 9 configured runtime cases pass, confirming the exact linked artifact behaves as expected. The native optimization budget and optimized LLVM contract are fully satisfied.

## Verification matrix
- Source semantics and expected result: PASS. The Weave source defines `fib` to return `n` if `n <= 1`, otherwise iteratively compute Fibonacci up to `n`. `main` reads `WEAVE_AUDIT_N`, defaults to 10 if missing or out-of-range (0..46), and returns `fib(n)`. Runtime cases confirm expected exit codes (e.g., `n=10` yields 55, `n=12` yields 144).
- Weave-to-WIR semantic preservation: PASS. The WIR projection preserves all source functions, parameters, return types, control flow (if/else, while), and operations (e.g., `le_i32`, `add_i32`, `call_ptr`, `call_i32`).
- WIR-to-raw-LLVM semantic preservation: PASS. Raw LLVM IR maps WIR operations to valid LLVM instructions (e.g., `icmp sle`, `add i32`, `call ptr @getenv`). Stack allocations (`alloca`) correctly implement mutable local variables.
- Raw LLVM validity, SSA, and control flow: PASS. The raw LLVM IR is valid SSA. All instructions are well-typed, basic blocks are properly terminated, and analysis JSON confirms 0 `poison_uses` and 0 `undef_uses`.
- Optimized LLVM semantic preservation: PASS. The optimized LLVM IR inlines `fib`, promotes stack variables to `phi` nodes, and folds bounds checks. The `freeze i32 %t3` correctly handles potential `undef` from `atoi` on non-numeric input, preserving safety. The loop structure accurately computes Fibonacci.
- Configured optimized LLVM metrics, functions, calls, and memory traffic: PASS. The optimized LLVM budget passes. Observed metrics (20 instructions, 6 basic blocks, 2 calls, 0 loads/stores/allocas) are within limits. Required calls (`atoi`, `getenv`) and defined functions (`main`) are present.
- Integer signedness, overflow, shifts, and comparisons: PASS. The source limits `n` to 0..46, preventing 32-bit overflow in `fib` (max value 1836311903). The optimized LLVM uses `icmp ult` for the upper bound check (safe for non-negative `atoi` results) and `icmp ult` for the `n < 2` check, which is equivalent to `icmp slt` for non-negative values. The loop uses `icmp sgt` for the loop exit condition.
- Calls, return values, ABI, stack alignment, and register use: PASS. The native code follows the System V ABI. `getenv` and `atoi` are called via PLT with arguments in `%rdi`. `main` returns its result in `%eax`. Stack alignment is maintained (8 bytes for `%rbx` spill, 16-byte alignment preserved).
- Memory safety, lifetime, leaks, and undefined behavior: PASS. The program uses only stack memory and read-only string literals. `getenv` result is null-checked. `atoi` result is frozen to prevent `undef` propagation. No heap allocations or leaks exist.
- Target compatibility and native instruction validity: PASS. The target assembly and disassembly show valid x86_64 instructions (e.g., `leaq`, `callq`, `cmpl`, `jle`, `addl`). The code is compatible with the specified `x86_64-pc-linux-gnu` target and glibc 2.39.
- Native runtime cases and expected observable behavior: PASS. All 9 runtime cases in the analysis JSON pass. The exact linked executable (SHA256 `8096a9e86313b02d5038903d99f134a5e00416c11de2752492f204fc927fec8d`) produces the expected exit codes for missing input, 0, 1, 2, 10, 12, -1, 47, and "abc".
- Configured native limits, required call targets, and loop backedges: PASS. The native budget passes. `main` has 25 instructions (limit 32), 2 direct calls (limit 2), 1 backward conditional branch (limit 1), and 1 padding instruction (limit 1). Required calls (`atoi@plt`, `getenv@plt`) are present.
- Compiler-generated overhead remaining in optimized LLVM and final native code: PASS. The optimized LLVM is clean, with no identity operations, dead stores, or redundant memory traffic. The native code is compact, using registers efficiently and avoiding unnecessary stack spills within the loop. The loop body is 7 instructions, which is optimal for this iterative Fibonacci implementation.

## Blocking findings
None found.

## Non-blocking opportunities
- The raw LLVM IR uses `alloca` and `load`/`store` for local variables, which is standard for unoptimized IR but fully optimized away by LLVM. No action needed.
- The loop vectorization passes (`loop-vectorize`, `slp-vectorizer`) report missed optimizations, which is expected for a loop with a data dependency that cannot be vectorized. No action needed.
- The native code uses `movl %eax, %esi` / `movl %edx, %eax` / `addl %esi, %eax` in the loop body, which is a standard 2-address instruction sequence. A 3-address architecture or different register allocation might reduce this, but it is not a compiler-generated overhead defect.

## Suggested verification
- Add a runtime case for `WEAVE_AUDIT_N=46` to verify the upper boundary of the non-overflow range (expected exit code 1836311903, modulo 256 if applicable, though the source returns `i32` and the harness checks exit code).
- Add a runtime case for `WEAVE_AUDIT_N=3` to verify a small non-trivial Fibonacci number (expected exit code 2).
- Consider adding a test for an empty string input (`WEAVE_AUDIT_N=""`) to verify `atoi` returns 0 and the program exits with 0.
