...
- **Report content SHA-256:** `b97fbc176018a2e7d22524337e5981fc3a64c48019d567f224b443690198ef83`
- **Audit timestamp (UTC):** `2026-07-28T14:18:09+00:00`
- **Re-audit no later than (UTC):** `2026-08-27T14:18:09+00:00`
...
- **Audited source Git SHA:** `22eedfe3ee9ae44a316f2a27d2a7742fe05138c3`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `22eedfe3ee9ae44a316f2a27d2a7742fe05138c3`
- **Auditor content SHA-256:** `b168c5707e84422cc68bd808845b9a382ef6860399d59f160def49c150177fcb`
...
- **LLM max tokens:** `4096`
- **LLM temperature:** `0.0`
- **LLM prompt SHA-256:** `fc14acde73adda6bfc797ba170f2495aef91800c297db822cf607cbb9fdae6b7`
- **LLM request SHA-256:** `c2c96a8333f1b162bfd3cf097263ef0ceeb8c52a6007a62c398ce29815a5830b`
- **Provider-reported model:** `z-ai/glm-5.2`
- **Provider response ID:** `chatcmpl-5c857680-0231-49e2-ab71-6af8719b9f0b`
- **Provider system fingerprint:** `unavailable`
- **Provider finish reason:** `stop`
- **Provider created (Unix):** `1785248290`
- **Provider prompt tokens:** `19259`
- **Provider completion tokens:** `1181`
- **Provider total tokens:** `20440`
- **GitHub run ID:** `30367593735`
- **GitHub workflow SHA:** `dd584c47dfaba64ea9fc53de4227a33269490064`
...
- `build_manifest` — SHA-256 `00139204a8c812754073686ce8f952f128601ee00a36e90a8f34e6587dcebee3`
...
- `llvm` — SHA-256 `36ea2f0a44806676bb90fe1b2788b66b0f72e6b28fe72a64cc07e05a2d50b354`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-l4LveH/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-j9o2ujxr/.audit.loupe.bsybarh3/artifacts/program",
...
## Summary
The final native program for `docs/audit/fibonacci_runtime.weave` is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The source semantics are preserved through cleaned WIR, raw LLVM, optimized LLVM, target assembly, and linked executable disassembly. The runtime matrix of 9 cases passes completely, matching expected exit codes. The native optimization budget is respected, with `main` containing 25 instructions (limit 32), 1 backward conditional branch (limit 1), and required direct calls to `getenv@plt` and `atoi@plt`. The final assembly instructionally traces the expected source behavior, including the input-dependent Fibonacci loop, with no substantial compiler-generated overhead remaining.

## Verification matrix
- Source semantics and expected result: PASS. The Weave source defines a runtime Fibonacci calculator reading `WEAVE_AUDIT_N`, defaulting to 10, clamping to 0..46, and returning `fib(n)`. The runtime matrix in the analysis JSON confirms all 9 cases pass with expected exit codes (e.g., `ten` returns 55, `twelve` returns 144).
- Weave-to-WIR semantic preservation: PASS. The WIR projection matches the source structure, preserving `getenv`/`atoi` externs, `fib` logic, `main` flow, and all comparisons (`le_i32`, `lt_i32`, `gt_i32`).
- WIR-to-raw-LLVM semantic preservation: PASS. Raw LLVM IR implements the WIR exactly, including `icmp sle`, `icmp slt`, `icmp sgt`, and the `while.cond1`/`while.body1` loop structure.
- Raw LLVM validity, SSA, types, and control flow: PASS. The raw LLVM uses valid SSA, correct `i32`/`ptr` types, and well-formed branches. The analysis JSON reports 0 `poison_uses` and 0 `undef_uses`.
- Optimized LLVM semantic preservation: PASS. The optimized LLVM inlines `fib` into `main`, preserves the loop via `while.body1.i`, and correctly handles the `freeze i32 %t3` to prevent poison propagation. The `ult` and `ugt` checks are valid optimizations given the source's clamping logic.
- Integer signedness, overflow, shifts, and comparisons: PASS. The source restricts `n` to 0..46, preventing `i32` overflow in `fib`. The optimized LLVM's use of `ult`/`ugt` is safe because negative inputs are clamped to 10. The final assembly uses `cmpl $46, %eax` and `ja` (unsigned above) and `cmpl $2, %eax` and `jb` (unsigned below), matching the optimized LLVM.
- Calls, return values, ABI, stack alignment, and register use: PASS. The assembly follows the System V AMD64 ABI: `getenv` and `atoi` receive their first argument in `rdi`, and return values are in `rax`/`eax`. `main` saves `rbx` and uses it for `n`, maintaining stack alignment.
- Memory safety, lifetime, leaks, and undefined behavior: PASS. The program uses only stack allocations and string literals. The `freeze` instruction in optimized LLVM prevents UB from `atoi` returning a poison value. No memory leaks or unsafe accesses are present.
- Target compatibility and native instruction validity: PASS. The target assembly and disassembly use valid x86_64 instructions (`leaq`, `callq`, `movl`, `cmpl`, `addl`, `incl`, `jle`, `ja`, `jb`, `retq`). The target triple is `x86_64-pc-linux-gnu`.
- Native runtime cases and expected observable behavior: PASS. The runtime matrix in the analysis JSON shows all 9 cases pass, including `missing-input-defaults-to-ten`, `zero`, `one`, `two`, `ten`, `twelve`, `negative-falls-back`, `too-large-falls-back`, and `non-numeric-atoi-zero`.
- Configured native limits, required call targets, and loop backedges: PASS. The native budget in the analysis JSON confirms `main` has 25 instructions (max 32), 1 backward conditional branch (max 1), and required direct calls to `atoi@plt` and `getenv@plt`. The budget `passed` field is `true`.
- Compiler-generated overhead remaining in final native code: PASS. The final assembly is compact, with the loop body consisting of 6 instructions (`movl`, `movl`, `addl`, `incl`, `movl`, `cmpl`/`jle`). The `nopw` padding is standard alignment and within the allowed padding budget. No avoidable overhead remains.

## Blocking findings
None found.

## Non-blocking opportunities
None found. The compiler successfully inlines `fib` and optimizes the code to a compact, efficient loop. The raw LLVM stack traffic is fully eliminated in the optimized LLVM and final assembly.

## Suggested verification
- Add a runtime case for `WEAVE_AUDIT_N=46` to explicitly test the upper boundary of the Fibonacci calculation (expected exit code 1836311903).
- Add a runtime case for `WEAVE_AUDIT_N=3` to test a non-trivial loop iteration count (expected exit code 2).
- Add a runtime case for a large non-numeric string (e.g., `WEAVE_AUDIT_N=999999999999999999999999`) to verify `atoi` behavior and clamping.
