...
- **Report content SHA-256:** `a4c450d339deea1a8fecffd7fa1fdc949be51ec038734c4083af601212e1ebfb`
- **Audit timestamp (UTC):** `2026-07-28T12:21:17+00:00`
- **Re-audit no later than (UTC):** `2026-08-27T12:21:17+00:00`
...
- **Audited source Git SHA:** `1a01d5b433372f0061a877a537a007c9c1db421c`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `1a01d5b433372f0061a877a537a007c9c1db421c`
- **Auditor content SHA-256:** `3e949ac7a96bb1b8eb63166c7ec945d3ceba1f398c90e1dee0273494fcf48d2f`
...
- **LLM max tokens:** `4096`
- **LLM temperature:** `0.0`
- **LLM prompt SHA-256:** `8688c1fdcd50c716b519cdf1f434285c72469510a49e2a5d25b68ebe0de26b8f`
- **LLM request SHA-256:** `d3e36231e223e824cdd0acbf2f39291ed33e2da18a8011e2df40c115d0b6fbfc`
- **Provider-reported model:** `z-ai/glm-5.2`
- **Provider response ID:** `chatcmpl-243f99b7-2322-4668-85bc-1b95dd4ad29b`
- **Provider system fingerprint:** `unavailable`
- **Provider finish reason:** `stop`
- **Provider created (Unix):** `1785241277`
- **Provider prompt tokens:** `18326`
- **Provider completion tokens:** `1210`
- **Provider total tokens:** `19536`
- **GitHub run ID:** `30358511252`
- **GitHub workflow SHA:** `f2b9e2d861a8a9cd990b984dfe01c71975b7acab`
...
- `build_manifest` — SHA-256 `d939bfcb4f63a7e20f534fe0a42ea9ba453abd565a808ac096ec0212ddd4ed58`
...
- `llvm` — SHA-256 `ae094807b2575f790945df0bd468e54a09a19da6195655a204f0b5d57230410c`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-KKAq4b/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-e1hxabh9/.audit.loupe.4dolkhoa/artifacts/program",
...
## Summary
The final native program for `docs/audit/fibonacci_runtime.weave` is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The source defines a runtime Fibonacci calculator that reads `WEAVE_AUDIT_N`, clamps the value to `0..46` (defaulting to 10 if missing, negative, too large, or non-numeric), and computes the nth Fibonacci number iteratively. The WIR preserves these semantics exactly. Raw LLVM IR is valid SSA with correct control flow and types. Optimized LLVM inlines `fib` into `main`, promotes stack variables to registers, and preserves the loop logic. The final x86_64 assembly matches the optimized LLVM, uses correct ABI conventions for `getenv` and `atoi`, and returns the result in `%eax`. The runtime matrix of 9 cases all passed, confirming the linked executable behaves as expected.

## Verification matrix
- Source semantics and expected result: PASS. The source defines `fib` with a base case (`n <= 1` returns `n`) and an iterative loop, and `main` clamps input to `0..46` (defaulting to 10). Runtime cases confirm expected results (e.g., `n=10` yields exit code 55, `n=12` yields 144).
- Weave-to-WIR semantic preservation: PASS. The WIR projection matches the source: `fib` uses `le_i32`, `add_i32`, and `main` uses `ne_ptr`, `lt_i32`, `gt_i32` with correct control flow and variable assignments.
- WIR-to-raw-LLVM semantic preservation: PASS. Raw LLVM IR mirrors the WIR: `fib` uses `icmp sle`, `add`, and `main` uses `icmp ne`, `icmp slt`, `icmp sgt` with correct branching and stores/loads.
- Raw LLVM validity, SSA, types, and control flow: PASS. The IR is valid SSA with proper `alloca`, `store`, `load`, and `br` instructions. Types (`i32`, `ptr`) are correct. Control flow edges are valid.
- Optimized LLVM semantic preservation: PASS. The optimized IR inlines `fib`, promotes variables to `phi` nodes, and preserves the loop logic. The `freeze` instruction on `atoi`'s result prevents undefined behavior from uninitialized memory, and the `icmp ugt` correctly handles the clamped range.
- Integer signedness, overflow, shifts, and comparisons: PASS. The source restricts input to `0..46`, preventing `i32` overflow in `fib` (max result 1836311903 for `n=46`). The optimized LLVM uses `icmp ult` for the `n < 2` check, which is safe because `n` is non-negative after clamping. The assembly uses `cmpl $46, %eax` (`ja`) and `cmpl $2, %eax` (`jb`), matching the unsigned comparisons.
- Calls, return values, ABI, stack alignment, and register use: PASS. The assembly uses `leaq .L.str0(%rip), %rdi` and `callq getenv@PLT` (System V ABI). `atoi` is called with `movq %rax, %rdi`. `main` returns via `retq` with the result in `%eax`. Stack is aligned via `pushq %rbx` (8 bytes spill).
- Memory safety, lifetime, leaks, and undefined behavior: PASS. No dynamic allocation is used. The `freeze` instruction in optimized LLVM prevents potential UB from `atoi` returning an uninitialized value. The string literal `WEAVE_AUDIT_N` is a properly null-terminated global constant.
- Target compatibility and native instruction validity: PASS. The assembly targets `x86_64-pc-linux-gnu` and uses valid instructions (`leaq`, `callq`, `cmpl`, `addl`, `incl`, `jle`, `jb`, `ja`, `retq`). The `.note.GNU-stack` section is present, marking the stack as non-executable.
- Native runtime cases and expected observable behavior: PASS. All 9 runtime cases passed, including missing input, zero, one, two, ten, twelve, negative, too-large, and non-numeric inputs. Exit codes match expected Fibonacci values or fallback behavior.
- Compiler-generated overhead remaining in final native code: PASS. The optimized LLVM and final assembly contain no unnecessary overhead. The `fib` loop is compact (6 instructions in the loop body). The `fib` function was inlined into `main` as expected. The loop was not vectorized, but this is a source-level algorithm limitation (value used outside loop), not a compiler defect.

## Blocking findings
None found.

## Non-blocking opportunities
- The `fib` loop could not be vectorized because the Fibonacci recurrence is sequential. This is a source-level algorithm cost, not compiler-generated overhead. A closed-form or matrix exponentiation approach would be faster for large `n`, but the current iterative approach is optimal for the constrained input range (`0..46`).
- The raw LLVM IR uses `alloca` and `store`/`load` for local variables, but these are fully optimized into registers in the final code. This is not a defect.

## Suggested verification
- Add a runtime case for `n=46` to confirm the maximum Fibonacci value (1836311903) is returned correctly without overflow.
- Add a runtime case for `n=3` to verify the loop executes at least once and returns 2.
- Verify the stack alignment of `main` at runtime using a debugger or dynamic analysis tool to ensure it remains 16-byte aligned before external calls (`getenv`, `atoi`).
