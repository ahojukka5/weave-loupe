...
- **Audit timestamp (UTC):** `2026-07-27T21:21:14+00:00`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-couAGh/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-c9yidh2k/.audit.loupe.z3fqtx7o/artifacts/program",
...
## Summary
The final native program is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The Weave source defines a runtime-input Fibonacci program that reads `WEAVE_AUDIT_N`, clamps the value to `[0, 46]` (falling back to 10), and computes Fibonacci iteratively. The cleaned WIR preserves these semantics exactly. Raw LLVM IR is valid SSA with correct control flow and memory operations. Optimized LLVM inlines `fib` into `main`, promotes all stack slots to registers, and preserves the source semantics. The target assembly and linked executable disassembly show a compact, ABI-compliant `main` that correctly calls `getenv` and `atoi` via the PLT, clamps the input, and executes an input-dependent Fibonacci loop. No undefined behavior, memory unsafety, ABI violations, or substantial compiler-generated overhead remain in the final native code.

## Verification matrix
- Source semantics and expected result: PASS. Weave source (`docs/audit/fibonacci_runtime.weave`) defines `fib` with base case `n <= 1` returning `n`, and an iterative loop for `n >= 2` returning the computed Fibonacci value. `main` reads `WEAVE_AUDIT_N`, defaults to 10, clamps to `[0, 46]`, and returns `fib(n)`. Expected behavior is fully preserved through all stages.
- Weave-to-WIR semantic preservation: PASS. WIR projection matches source: `fib` uses `le_i32 n (const_i32 1)` for the base case, initializes `previous=0`, `current=1`, `index=2`, loops while `le_i32 index n`, and returns `current`. `main` calls `getenv`, checks `ne_ptr input (const_null)`, calls `atoi`, clamps with `lt_i32 n 0` and `gt_i32 n 46`, and returns `fib(n)`.
- WIR-to-raw-LLVM semantic preservation: PASS. Raw LLVM IR declares `getenv` and `atoi` with correct signatures. `@main` allocates `%n.addr`, stores 10, calls `getenv` on `@.str0` (`"WEAVE_AUDIT_N\0"`), conditionally calls `atoi`, clamps with `icmp slt` and `icmp sgt`, and returns `call i32 @fib(i32 %t8)`. `@fib` matches WIR with `icmp sle i32 %n, 1`, loop `icmp sle i32 %t1, %n`, and `add i32` operations.
- Raw LLVM validity, SSA, and control flow: PASS. All SSA values are properly numbered, basic blocks have valid terminators (`br`, `ret`), `alloca` instructions are valid, and `getelementptr` correctly indexes `[14 x i8]`. Analysis JSON confirms 0 `poison_uses` and 0 `undef_uses`.
- Optimized LLVM semantic preservation: PASS. `fib` is inlined into `main` (optimization record confirms `Inlined` with cost -15005). Stack slots are promoted to registers via `phi` nodes. The `freeze i32 %t3` prevents UB from `atoi`'s potentially undef return. The unsigned comparison `icmp ugt i32 %t3.fr, 46` correctly combines the two source clamps (`< 0` and `> 46`) since `n` is already non-negative after the first check. The `icmp ult i32 %t3.fr, 2` correctly handles the inlined `fib` base case.
- Integer signedness, overflow, shifts, and comparisons: PASS. Raw LLVM uses `icmp slt` and `icmp sgt` for signed comparisons matching source `lt_i32` and `gt_i32`. `fib` uses `icmp sle` for `le_i32`. Optimized LLVM uses `icmp ugt` and `icmp ult` which are valid because the value is already non-negative. `add i32` in the loop is correct; for `n <= 46`, the maximum Fibonacci value is 1836311903, which fits in `i32` (max 2147483647), so no signed overflow occurs.
- Calls, return values, ABI, stack alignment, and register use: PASS. Target assembly uses `rdi` for the first argument to `getenv` and `atoi` (System V AMD64 ABI). `getenv` returns in `rax`, correctly tested with `testq %rax, %rax`. `atoi` returns in `eax`, correctly moved to `%ebx`. `main` returns `i32` in `%eax` (via `%ebx` or `%eax` directly). Stack is 16-byte aligned: `pushq %rbx` decrements `%rsp` by 8, and no further stack allocation is needed, maintaining alignment for calls. `%rbx` is callee-saved and properly restored before `retq`.
- Memory safety, lifetime, leaks, and undefined behavior: PASS. No dynamic allocation; all memory access is to stack slots (raw LLVM) or registers (optimized LLVM). `@.str0` is a valid null-terminated string constant. `getenv` result is null-checked before `atoi` call. `freeze` in optimized LLVM prevents UB from `atoi`'s undef return. No memory leaks or lifetime issues.
- Target compatibility and native instruction validity: PASS. Target assembly specifies `x86_64-pc-linux-gnu` (from optimized LLVM datalayout/triple). Instructions (`pushq`, `leaq`, `callq`, `movl`, `testq`, `cmpl`, `ja`, `jb`, `jle`, `addl`, `incl`, `popq`, `retq`) are all valid x86-64 instructions. Linked disassembly confirms correct PLT stubs for `getenv` and `atoi` and a valid `main` at `0x1150`.
- Compiler-generated overhead remaining in final native code: PASS. Final `main` is 25 instructions with 8 stack bytes (only `%rbx` spill). The loop body is 7 instructions (`movl`, `movl`, `addl`, `incl`, `movl`, `cmpl`, `jle`) with no redundant moves or spills. The `nopw` at `0x1187` is loop alignment padding, not overhead. No avoidable compiler-generated overhead remains.

## Blocking findings
None found.

## Non-blocking opportunities
- The loop is not vectorized (optimization record: `NonReductionValueUsedOutsideLoop`, `CantComputeNumberOfIterations`). This is a source algorithm characteristic (Fibonacci is inherently sequential due to loop-carried dependencies), not compiler-generated overhead. No action needed.
- Raw LLVM uses `alloca` and `store`/`load` for local variables, but these are fully optimized away in the optimized LLVM and final native code. This is not a defect.
- The `freeze i32 %t3` in optimized LLVM is a necessary safety measure to prevent UB from `atoi`'s potentially undef return, not overhead.

## Suggested verification
- Runtime test: set `WEAVE_AUDIT_N=0`, `1`, `2`, `10`, `46` and verify the program output matches expected Fibonacci values (0, 1, 1, 55, 1836311903).
- Runtime test: set `WEAVE_AUDIT_N=47` and verify the program falls back to 10 (output 55).
- Runtime test: set `WEAVE_AUDIT_N=-1` and verify the program falls back to 10.
- Runtime test: unset `WEAVE_AUDIT_N` and verify the program defaults to 10.
- Verify stack alignment at `callq` instructions by inspecting `%rsp` value modulo 16 in a debugger.
