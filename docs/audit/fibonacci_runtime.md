...
- **Audit timestamp (UTC):** `2026-07-27T23:15:32+00:00`
- **Audited source Git SHA:** `58d968ed555fdc33a3cfc5df957d8c6be54a1efe`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `58d968ed555fdc33a3cfc5df957d8c6be54a1efe`
...
- **GitHub run ID:** `30313454568`
- **GitHub workflow SHA:** `329b7e1f0f7176c79aabb20214f4a98da804306f`
...
- `build_manifest` — SHA-256 `ffc9cc076464abdcf36b2f6d08826eb9219e589ffcb33d4bf05b657a2fb4f5b9`
...
- `llvm` — SHA-256 `ecfd8dcf19e0519943abc8acdcc83b3b8a0db4c594e567fa268ab69e4e05497d`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-5fEnBD/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-15rsct_m/.audit.loupe.4pwh01ci/artifacts/program",
...
## Summary
The final native program is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The Weave source implements a runtime-input Fibonacci program that reads `WEAVE_AUDIT_N`, clamps the value to `0..46` with a fallback of `10`, and returns `fib(n)`. The WIR preserves these semantics, and the raw LLVM IR correctly lowers them with valid SSA, typed memory operations, and proper control flow. The optimized LLVM inlines `fib` into `main`, promotes stack locals to SSA registers, and preserves the loop semantics. The target assembly and linked executable disassembly show a valid x86_64 implementation with correct signed/unsigned comparisons, proper SysV ABI usage for `getenv` and `atoi`, and no remaining compiler-generated overhead. The runtime matrix of 9 cases all passed, confirming the final artifact behaves as expected.

## Verification matrix
- Source semantics and expected result: PASS. The Weave source defines `fib` with a base case for `n <= 1` and an iterative loop, and `main` reads `WEAVE_AUDIT_N`, clamps to `0..46` with fallback `10`, and returns `fib(n)`. The runtime matrix confirms expected results (e.g., `n=10` -> exit code `55`, `n=12` -> `144`).
- Weave-to-WIR semantic preservation: PASS. The WIR projection matches the source: `fib` uses `le_i32 n (const_i32 1)` for the base case and a `while` loop with `add_i32`; `main` calls `getenv`, checks `ne_ptr`, calls `atoi`, and clamps with `lt_i32` and `gt_i32`.
- WIR-to-raw-LLVM semantic preservation: PASS. Raw LLVM uses `icmp sle` for `le_i32`, `icmp ne` for `ne_ptr`, `icmp slt` for `lt_i32`, `icmp sgt` for `gt_i32`, and `add i32` for `add_i32`, matching WIR semantics.
- Raw LLVM validity, SSA, types, and control flow: PASS. The IR is well-formed with typed `alloca`, `load`, `store`, and `br` instructions. SSA is valid, with no `poison_uses` or `undef_uses` per the analysis JSON.
- Optimized LLVM semantic preservation: PASS. The optimized LLVM inlines `fib`, promotes locals to `phi` nodes, and uses `icmp ult`/`ugt` for the clamping checks (valid since `n` is non-negative after `atoi` and the checks are equivalent for non-negative values). The loop logic is preserved.
- Integer signedness, overflow, shifts, and comparisons: PASS. The source clamps `n` to `0..46`, preventing `fib` overflow (max `fib(46)` is within `i32` range). The optimized LLVM uses `ult`/`ugt` for clamping, which is equivalent to `slt`/`sgt` for non-negative values. The assembly uses `cmpl $46, %eax; ja` and `cmpl $2, %eax; jb`, correctly implementing unsigned comparisons.
- Calls, return values, ABI, stack alignment, and register use: PASS. The assembly uses `rdi` for the `getenv` and `atoi` arguments, `rax` for return values, and `ebx` as a callee-saved register with proper `pushq`/`popq` prologue/epilogue. Stack alignment is maintained.
- Memory safety, lifetime, leaks, and undefined behavior: PASS. The program uses a string literal for `getenv` and passes the returned pointer to `atoi` after a null check. No dynamic allocation is present. The analysis JSON reports `poison_uses: 0` and `undef_uses: 0`.
- Target compatibility and native instruction validity: PASS. The assembly targets `x86_64-pc-linux-gnu` and uses valid instructions (`leaq`, `callq`, `testq`, `cmpl`, `addl`, `jle`, `retq`). The linked executable disassembly confirms a valid ELF64 binary.
- Native runtime cases and expected observable behavior: PASS. All 9 runtime cases passed, including missing input, zero, one, two, ten, twelve, negative, too-large, and non-numeric input, matching the expected exit codes.
- Compiler-generated overhead remaining in final native code: PASS. The final assembly contains no redundant instructions or spills. The loop is compact with 7 instructions, and the optimizer removed all stack traffic from `fib`.

## Blocking findings
None found.

## Non-blocking opportunities
- The loop-vectorize and SLP-vectorizer passes missed vectorization opportunities, as noted in the optimization record. This is a performance opportunity, not a defect, as the loop is inherently sequential due to the Fibonacci recurrence.
- The raw LLVM uses `alloca` for locals in `fib` and `main`, which are promoted to SSA registers in optimized LLVM. This is expected and not a defect.

## Suggested verification
- Add a runtime case for `n=46` to verify the upper bound of the Fibonacci calculation.
- Add a runtime case for `n=3` to verify the loop entry condition.
- Add a test with a malformed numeric string (e.g., `"12abc"`) to verify `atoi` behavior.
