...
- **Audit timestamp (UTC):** `2026-07-28T00:24:46+00:00`
- **Re-audit no later than (UTC):** `2026-08-27T00:24:46+00:00`
...
- **Audited source Git SHA:** `950b33851558f66a1eb18bc9ea506135ef064759`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `950b33851558f66a1eb18bc9ea506135ef064759`
...
- **GitHub run ID:** `30317095347`
- **GitHub workflow SHA:** `d7389a91f85e2d8c613a667e680b4ad827c4e569`
...
- `build_manifest` — SHA-256 `591bbf72ba6dfba37be1a168dbdc387c5687f478e8362690ea757d1163f1f730`
...
- `llvm` — SHA-256 `f95477972e133fd19cb28f1d644a97aca62666649e462906079c4cc821291e08`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-xJadjk/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-cbs_8a9h/.audit.loupe.5betqxka/artifacts/program",
...
## Summary
The final native program is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The Weave source defines a runtime-driven Fibonacci loop that reads `WEAVE_AUDIT_N`, clamps the value to `0..46` with a fallback of `10`, and returns `fib(n)` as the process exit code. The cleaned WIR preserves that structure exactly. Raw LLVM lowers it to valid SSA with correct signed comparisons and memory behavior. Optimized LLVM inlines `fib`, promotes locals to registers, and folds the clamping checks into unsigned comparisons that are equivalent on the non-negative domain. The linked x86_64 executable uses the System V ABI correctly for `getenv` and `atoi`, preserves `rbx`, returns the result in `eax`, and contains a compact input-dependent loop with no avoidable overhead. The configured runtime matrix executed the exact linked artifact and all 9 cases passed.

## Verification matrix
- Source semantics and expected result: PASS. Source `docs/audit/fibonacci_runtime.weave` defines `fib` returning `n` for `n <= 1`, otherwise iterating `previous/current` from index `2` to `n`, and `main` returning `fib(clamped n)` as `i32`. Expected exit codes in `docs/audit/fibonacci_runtime.audit.json` match Fibonacci values for `0,1,2,10,12`, fallback `55` for missing/negative/too-large, and `0` for non-numeric `atoi`.
- Weave-to-WIR semantic preservation: PASS. WIR projection preserves `fib` early return, `let previous/current/index`, `while (le_i32 index n)`, and `main` getenv/atoi/clamp sequence with `lt_i32` and `gt_i32`.
- WIR-to-raw-LLVM semantic preservation: PASS. Raw LLVM `@main` allocas `n`, stores `10`, calls `getenv`, conditionally calls `atoi`, applies `icmp slt` and `icmp sgt` clamps, then calls `@fib`. `@fib` uses `icmp sle n, 1`, loop condition `icmp sle %t1, %n`, and `add i32` for `next` and `index`.
- Raw LLVM validity, SSA, types, and control flow: PASS. SSA is well formed, types are `i32`/`ptr`, basic blocks are terminated correctly, no `undef`/`poison` uses per analysis JSON (`poison_uses: 0`, `undef_uses: 0`).
- Optimized LLVM semantic preservation: PASS. `fib` is inlined; locals become PHI nodes; `icmp ult %t3.fr, 2` and `icmp ugt %t3.fr, 46` are equivalent to signed checks because `atoi` result is frozen and negative values route to fallback `10` before the unsigned comparison. Loop PHI and `add i32` match source iteration.
- Integer signedness, overflow, shifts, and comparisons: PASS. Source clamps to `0..46`, so `fib` loop and `add i32` cannot overflow `i32` (`fib(46)=1836311903`). Optimized unsigned comparisons are valid on the non-negative domain. Native code uses `cmpl $0x2e, %eax; ja` and `cmpl $0x2, %eax; jb`, correct for unsigned range checks after negative fallback.
- Calls, return values, ABI, stack alignment, and register use: PASS. `getenv`/`atoi` called through PLT with `rdi` argument and `eax` return. `main` saves/restores `rbx`, stack is 16-byte aligned at calls after `pushq %rbx`, result returned in `eax`. No callee-saved register clobber.
- Memory safety, lifetime, leaks, and undefined behavior: PASS. No heap allocation. `getenv` null check precedes `atoi`. String literal is NUL-terminated. No out-of-bounds access. Optimized LLVM has no `load`/`store`/`alloca`.
- Target compatibility and native instruction validity: PASS. Target triple `x86_64-pc-linux-gnu`; disassembly is valid ELF64 with standard PLT/GOT stubs, `_start`, and `main` at `0x1150`. Instructions are legal x86_64.
- Native runtime cases and expected observable behavior: PASS. Runtime matrix has 9 cases, all `passed: true`, executable SHA matches bundle, sidecar SHA matches metadata. Cases cover missing, `0`, `1`, `2`, `10`, `12`, `-1`, `47`, and `abc`.
- Compiler-generated overhead remaining in final native code: PASS. `main` is 25 instructions with one NOP for loop alignment. No spills, no redundant copies, no identity moves, no stack traffic in final code. Loop is input-dependent as required.

## Blocking findings
None found.

## Non-blocking opportunities
- The optimized LLVM uses `freeze i32 %t3` to guard against `atoi` returning undef on non-numeric input. This is safe and necessary, but the source comment could document that `atoi("abc")` returning `0` is the intended fallback behavior, which the runtime case `non-numeric-atoi-zero` confirms.
- The loop is not vectorized because the Fibonacci recurrence is inherently sequential. This is a source algorithm cost, not compiler overhead, and is correctly left as a scalar loop.
- Raw LLVM uses allocas and loads/stores for `n`, `previous`, `current`, `index`, and `next`. These are fully promoted by LLVM and do not appear in final native code, so they are not a defect.

## Suggested verification
- Add runtime cases for boundary values `46` and `45` to directly confirm the upper clamp edge and the largest in-range Fibonacci value.
- Add a runtime case for `WEAVE_AUDIT_N` set to an empty string to confirm `atoi("")` returns `0` and the program exits with `0`.
- Add a runtime case for a very large numeric string such as `999999999999` to confirm the `> 46` clamp triggers fallback `55` and no signed-comparison anomaly occurs.
- Optionally inspect the raw WIR hash-addressed export to confirm no hidden semantic tokens were altered beyond the review projection.
