...
- **Report content SHA-256:** `03c4bb2aa6d3a38f15e864a3dcacacf8c152a32cbdd45b7c1afb48eb844106c4`
- **Audit timestamp (UTC):** `2026-07-29T06:07:40+00:00`
- **Re-audit no later than (UTC):** `2026-08-28T06:07:40+00:00`
...
- **LLM max tokens:** `4096`
- **LLM temperature:** `0.0`
- **LLM prompt SHA-256:** `5f7e35f4d69d774f6b423e9d6cdef13d918556a231d69f160e82848abe10eba6`
- **LLM request SHA-256:** `78635d253601e5cf774e709ad3062a98cc4612f90d4a110fce7a8cb85ca61aa6`
- **Provider-reported model:** `z-ai/glm-5.2`
- **Provider response ID:** `chatcmpl-df244d40-d1b2-4f3c-aed1-69dcc64dbb15`
- **Provider system fingerprint:** `unavailable`
- **Provider finish reason:** `stop`
- **Provider created (Unix):** `1785305260`
- **Provider prompt tokens:** `19260`
- **Provider completion tokens:** `1065`
- **Provider total tokens:** `20325`
...
- `build_manifest` — SHA-256 `2c9d8f85faeba679d22774e15d3d4879c0a2642cad8dfcb93be7f78856a46078`
...
- `llvm` — SHA-256 `dd28d922fca05cbd5f52112c109413fff73b6fb5a64899e0723571b3c6198927`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-MvI0fD/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-q2uv1lso/.audit.loupe.ju640ai_/artifacts/program",
  "sources": [
    "docs/audit/fibonacci_runtime.weave"
  ]
}
...
## Summary
The final native program for `docs/audit/fibonacci_runtime.weave` is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The Weave source defines a runtime-dependent Fibonacci calculation that intentionally resists constant folding. This semantic is preserved through cleaned WIR, raw LLVM, and optimized LLVM, where the `fib` function is inlined into `main` but the input-dependent loop remains intact. The final x86_64 assembly correctly implements the loop using registers `%eax`, `%ecx`, `%edx`, and `%esi`, and satisfies the System V ABI by preserving `%rbx` across the `getenv` and `atoi` calls. All 9 configured runtime matrix cases pass against the exact linked executable, and the native optimization budget contract is fully satisfied with no substantial overhead remaining in the final machine code.

## Verification matrix
- Source semantics and expected result: PASS. The Weave source specifies reading `WEAVE_AUDIT_N`, defaulting to 10 if missing or out-of-range [0, 46], and computing Fibonacci iteratively. The runtime matrix confirms these exact semantics across 9 cases.
- Weave-to-WIR semantic preservation: PASS. The cleaned WIR accurately captures the `getenv`/`atoi` externs, the `fib` function logic, the `main` entry point, and all control flow and arithmetic operations present in the source.
- WIR-to-raw-LLVM semantic preservation: PASS. The raw LLVM IR maps the WIR statements directly to LLVM instructions, including `alloca` for local variables, `icmp` for conditions, and `br` for control flow, maintaining identical semantics.
- Raw LLVM validity, SSA, types, and control flow: PASS. The raw LLVM IR is well-formed, uses correct SSA naming, valid types (`i32`, `ptr`), and valid control flow edges with no unreachable blocks or invalid instructions.
- Optimized LLVM semantic preservation: PASS. The optimized LLVM IR inlines `fib` into `main`, promotes all stack allocations to registers, and uses `phi` nodes for the loop. The loop logic and input clamping remain semantically identical to the source.
- Integer signedness, overflow, shifts, and comparisons: PASS. The source limits input to 0..46, preventing 32-bit overflow in the Fibonacci sequence. The optimized LLVM correctly uses `icmp ugt` and `icmp ult` for the clamping logic, and the native code uses `cmpl` and `ja`/`jb` appropriately.
- Calls, return values, ABI, stack alignment, and register use: PASS. The native code follows the System V AMD64 ABI. `%rbx` is saved and restored. `getenv` and `atoi` are called via the PLT with arguments in `%rdi`, and return values are taken from `%rax`. The stack is 16-byte aligned at call sites.
- Memory safety, lifetime, leaks, and undefined behavior: PASS. The program reads a string literal and passes it to external functions. No dynamic memory is allocated. The optimized LLVM contains no `undef` or `poison` uses.
- Target compatibility and native instruction validity: PASS. The target assembly and linked disassembly use valid x86_64 instructions matching the `x86_64-pc-linux-gnu` triple and glibc 2.39 environment.
- Native runtime cases and expected observable behavior: PASS. The runtime matrix shows all 9 cases passing, with actual exit codes matching expected exit codes exactly.
- Configured native limits, required call targets, and loop backedges: PASS. The native budget is configured and passed. `main` has 25 instructions (max 32), 1 backward conditional branch (min 1, max 1), and required direct calls to `atoi@plt` and `getenv@plt` are present.
- Compiler-generated overhead remaining in final native code: PASS. The final native code is compact and efficient. The loop body consists of 6 essential instructions with 1 padding instruction for alignment. No unnecessary spills or overhead remains.

## Blocking findings
None found.

## Non-blocking opportunities
- The loop in the final native code uses `movl` instructions to shuffle values between `%eax`, `%edx`, and `%esi` due to the sequential dependencies in the Fibonacci recurrence. While this is an inherent property of the source algorithm, advanced register allocation or using `lea` for addition might save one instruction, though it is well within the native optimization budget.
- The LLVM optimization record notes that the loop was not vectorized. This is expected for a standard iterative Fibonacci implementation due to the loop-carried dependency, and vectorizing it would require a source-level algorithm change (e.g., using matrix exponentiation), which is outside the scope of compiler-generated overhead.

## Suggested verification
- Add a runtime case for `WEAVE_AUDIT_N=46` to explicitly verify the upper boundary of the non-clamped Fibonacci calculation.
- Add a runtime case for `WEAVE_AUDIT_N=45` to verify the largest Fibonacci number representable in a signed 32-bit integer before the clamp logic triggers.
