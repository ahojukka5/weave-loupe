...
- **Report content SHA-256:** `3486429eafba5ad4e02a92e30936f92411c978b576aa093fd09922e5be7609c3`
- **Audit timestamp (UTC):** `2026-07-28T11:35:06+00:00`
- **Re-audit no later than (UTC):** `2026-08-27T11:35:06+00:00`
...
- **Provider response ID:** `chatcmpl-28f32b11-7128-498e-ac31-f22ed456b50e`
...
- `build_manifest` — SHA-256 `60aec6b30e9bead484eb8be4b1608e396970a79d665f2194a4334a71deb9ae91`
...
- `llvm` — SHA-256 `017db67eb2fb33f4f9cd2d6bbd6182c8aa267b98e9ab5263423cfb56d99f124b`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-5LhQ5X/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-qzhdssbi/.audit.loupe.njypn1i2/artifacts/program",
...
## Summary
The final native program is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The source semantics require reading `WEAVE_AUDIT_N`, clamping the value to `0..46` with a fallback of 10, computing Fibonacci iteratively, and returning the result as the process exit code. The cleaned WIR, raw LLVM, optimized LLVM, target assembly, and linked disassembly all preserve this behavior. The optimized LLVM inlines `fib` and keeps the input-dependent loop, satisfying the corpus requirement. The final x86-64 assembly uses a compact register-only loop with no avoidable overhead. All 9 configured runtime cases pass against the hash-addressed sidecar.

## Verification matrix
- Source semantics and expected result: PASS — Weave source defines `fib` and `main` with getenv/atoi externs, clamping to 0..46, fallback 10, and returning `fib(n)`. Runtime cases confirm exit codes 0, 1, 55, 144, etc.
- Weave-to-WIR semantic preservation: PASS — WIR projection matches source structure, externs, comparisons, loop, and return.
- WIR-to-raw-LLVM semantic preservation: PASS — Raw LLVM declares `getenv`/`atoi`, implements `main` and `internal i32 @fib`, uses `icmp sle/slt/sgt`, and preserves the while loop.
- Raw LLVM validity, SSA, and control flow: PASS — Raw LLVM is valid, uses SSA values, well-formed basic blocks, and correct branches/returns.
- Optimized LLVM semantic preservation: PASS — Optimized LLVM inlines `fib`, uses `freeze i32 %t3`, `icmp ugt i32 %t3.fr, 46`, `icmp ult i32 %t3.fr, 2`, and a phi-based loop preserving semantics.
- Integer signedness, overflow, shifts, and comparisons: PASS — Comparisons match source: `le_i32`→`icmp sle`, `lt_i32`→`icmp slt`, `gt_i32`→`icmp sgt` in raw LLVM; optimized LLVM uses equivalent unsigned tests after range checks. `add i32` matches source `add_i32`.
- Calls, return values, ABI, stack alignment, and register use: PASS — `getenv`/`atoi` use `ptr` in `rdi`, return in `rax`; `main` saves/restores `rbx`, 8-byte stack, 16-byte aligned at call boundaries; `ret` returns `i32` in `eax`.
- Memory safety, lifetime, leaks, and undefined behavior: PASS — No allocations/leaks; `getenv` result null-checked before `atoi`; `freeze` avoids UB on `atoi` result; no out-of-bounds access.
- Target compatibility and native instruction validity: PASS — x86-64 instructions valid for `x86_64-pc-linux-gnu`; PLT calls correct; `.note.GNU-stack` present.
- Native runtime cases and expected observable behavior: PASS — All 9 cases pass: missing-input→55, 0→0, 1→1, 2→1, 10→55, 12→144, -1→55, 47→55, abc→0.
- Compiler-generated overhead remaining in final native code: PASS — No avoidable overhead in optimized LLVM or final code; loop is register-only with no spills/redundant copies.

## Blocking findings
None found.

## Non-blocking opportunities
- Raw LLVM uses `alloca`/`load`/`store` for locals, but these are fully optimized away in optimized LLVM and final code; non-blocking.
- Loop vectorization is missed, but this is inherent to the Fibonacci recurrence and not compiler-generated overhead; non-blocking.
- `atoi` is used per source; `strtol` would be safer for invalid input, but source semantics are preserved and runtime case `abc→0` passes; non-blocking.

## Suggested verification
- Add runtime cases for boundary values 2, 45, 46 to directly confirm loop edges.
- Add a case for large numeric input like `100` to confirm fallback to 10.
- Add a case for whitespace-prefixed numeric input to document `atoi` behavior.
