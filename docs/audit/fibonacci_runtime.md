...
- **Audit timestamp (UTC):** `2026-07-28T07:29:03+00:00`
- **Re-audit no later than (UTC):** `2026-08-27T07:29:03+00:00`
...
- **Audited source Git SHA:** `86ec36752f92ff656376d32150ef5b337d76d9e1`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `86ec36752f92ff656376d32150ef5b337d76d9e1`
...
- **GitHub run ID:** `30338507527`
- **GitHub workflow SHA:** `5fc001861a9f2704f1873d234b35f4eb1554878f`
...
- `build_manifest` — SHA-256 `b8634c1856056b5a89a6d83d4a0659199be30e4e10253c68533e87160bae677a`
...
- `llvm` — SHA-256 `435e81f6f63a26e0a4ca485a7928e77e6cc2b011dbb838321cb005dadcaa14e9`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-flljyS/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-aw_r_9gj/.audit.loupe.oxgqivgg/artifacts/program",
...
## Summary
The final native program is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The Weave source computes Fibonacci with runtime input clamped to 0..46, falling back to 10. The cleaned WIR preserves that semantics. Raw LLVM is valid SSA with correct control flow. Optimized LLVM inlines `fib`, promotes all locals to registers, and preserves the signed loop comparison. Target assembly and linked disassembly match the optimized LLVM and use correct x86_64 SysV ABI conventions for `getenv`, `atoi`, and `main`. The configured runtime matrix executed the exact linked artifact and all 9 cases passed, including boundary and fallback cases. No blocking defects were found.

## Verification matrix
- Source semantics and expected result: PASS. Weave source defines `fib(n)` returning `n` for `n <= 1`, otherwise iterating from index 2 to `n` inclusive; `main` reads `WEAVE_AUDIT_N`, falls back to 10 when missing, negative, or >46, and returns `fib(n)`. Runtime cases confirm expected exit codes: 0->0, 1->1, 2->1, 10->55, 12->144, missing->55, -1->55, 47->55, "abc"->0.
- Weave-to-WIR semantic preservation: PASS. WIR projection matches source: `le_i32 n 1` early return, `previous=0`, `current=1`, `index=2`, while `le_i32 index n`, `next=add_i32 previous current`, updates, `index+1`; `main` calls `getenv`, null check, `atoi`, `<0` and `>46` fallbacks, `fib` call.
- WIR-to-raw-LLVM semantic preservation: PASS. Raw LLVM `@fib` uses `icmp sle i32 %n, 1`, allocas for previous/current/index, while loop with `icmp sle i32 %t1, %n`, `add i32`, and returns current. `@main` calls `getenv`, `icmp ne ptr %t1, null`, `atoi`, `icmp slt i32 %t4, 0`, `icmp sgt i32 %t6, 46`, fallback stores to 10, and `call i32 @fib`.
- Raw LLVM validity, SSA, types, and control flow: PASS. SSA is valid, types are `i32`/`ptr`, basic blocks are well-formed, branches are explicit, no undef/poison uses. Analysis JSON reports 0 `undef_uses`, 0 `poison_uses`, 0 `invoke`.
- Optimized LLVM semantic preservation: PASS. `fib` is inlined into `main`; allocas and loads/stores are eliminated; loop is preserved with `phi` nodes for index/current/previous; `icmp sgt i32 %t8.i, %0` preserves signed loop exit; `freeze i32 %t3` safely handles `atoi` nondeterminism; `ult`/`ugt` range checks are valid because values are nonnegative after the `<0` fallback.
- Integer signedness, overflow, shifts, and comparisons: PASS. All arithmetic is `i32`; Fibonacci values for n<=46 fit in signed 32-bit (max 1836311903). Loop comparison `cmpl %ebx, %ecx; jle` is signed and matches `le_i32`. Range checks use unsigned comparisons on nonnegative values, equivalent to signed for the valid domain. No shifts present.
- Calls, return values, ABI, stack alignment, and register use: PASS. `getenv`/`atoi` called via PLT with `ptr` in `%rdi`; results in `%rax`. `main` aligns stack via `pushq %rbx` (8 bytes), preserving 16-byte alignment at calls per SysV ABI. Return value in `%eax`. `rbx` is callee-saved and restored before `retq`.
- Memory safety, lifetime, leaks, and undefined behavior: PASS. No heap allocation. String literal is read-only in `.rodata`. Null pointer from `getenv` is checked before `atoi`. `freeze` prevents poison propagation from `atoi`. No memory leaks or unsafe lifetime issues.
- Target compatibility and native instruction validity: PASS. Target triple is `x86_64-pc-linux-gnu`; instructions are valid x86_64; PLT/GOT linkage is standard for glibc; `.note.GNU-stack` present; `main` at 0x1150 matches `_start` argument.
- Native runtime cases and expected observable behavior: PASS. All 9 runtime cases passed with expected exit codes and empty stdout/stderr. Executable SHA256 matches bundle artifact. Sidecar hash matches.
- Compiler-generated overhead remaining in final native code: PASS. No redundant compiler-generated overhead remains. Loop is compact with 7 instructions; no spills inside loop; no identity moves; no unnecessary stack traffic in optimized code. Regalloc copies are outside the loop and do not constitute substantial overhead.

## Blocking findings
None found.

## Non-blocking opportunities
- The raw LLVM uses allocas and load/store traffic for `previous`, `current`, `index`, and `n`. This is expected unoptimized IR and is fully removed by LLVM mem2reg/inlining, so it is non-blocking.
- The loop was not vectorized because Fibonacci is inherently sequential. This is a source algorithm characteristic, not compiler-generated overhead.
- `getenv` and `atoi` are external libc calls and cannot be inlined. This is necessary for runtime input and not avoidable overhead.
- A `nopw` padding instruction exists at 0x1187 for loop alignment. This is a standard target alignment hint and not a semantic or overhead defect.

## Suggested verification
- Add a runtime case for `WEAVE_AUDIT_N=46` to confirm the upper boundary Fibonacci value 1836311903 fits in signed 32-bit and returns correctly.
- Add a runtime case for a very large positive integer string such as `999999999999` to confirm `atoi` overflow behavior and fallback logic.
- Add a runtime case for `WEAVE_AUDIT_N=3` to verify the first non-trivial loop iteration returns 2.
- Consider adding a case with whitespace-padded numeric input to document `atoi` skipping behavior.
