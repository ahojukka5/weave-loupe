...
- **Audit timestamp (UTC):** `2026-07-28T03:30:19+00:00`
- **Re-audit no later than (UTC):** `2026-08-27T03:30:19+00:00`
...
- **Audited source Git SHA:** `eda8e91656f789bf610edab4e321f33f45d80b09`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `eda8e91656f789bf610edab4e321f33f45d80b09`
...
- **GitHub run ID:** `30326032277`
- **GitHub workflow SHA:** `1ebfc203669462da52207d8c5ee82f790b2c0c7b`
...
- `build_manifest` — SHA-256 `f3e37133754d2131f5ffd4a977b32377e1171be7ba5d1d7bcd3b462955a29215`
...
- `llvm` — SHA-256 `fac623856d0411bc7afc3252ab0ae8ac5ed2f83ce488cfe68263c5083e5aa57c`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-1No79s/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-1bfqa5wv/.audit.loupe.4snn1ry3/artifacts/program",
...
## Summary
The final native program for `fibonacci-runtime` is correct, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The source semantics are preserved through the WIR, raw LLVM, optimized LLVM, and final native assembly. The runtime matrix configured against the linked executable passes all 9 cases, affirmatively proving that the observable behavior matches the expected sidecar values. The compiler successfully inlines the `fib` function and eliminates all stack traffic in the optimized LLVM, leaving a tight register-only loop in the final assembly.

## Verification matrix
- Source semantics and expected result: PASS. The Weave source defines a `fib` function that returns `n` if `n <= 1`, otherwise iteratively computes Fibonacci. `main` reads `WEAVE_AUDIT_N`, falls back to 10 if missing/negative/too large, and returns `fib(n)`. The runtime matrix confirms these exact semantics (e.g., input 10 yields 55, input 12 yields 144).
- Weave-to-WIR semantic preservation: PASS. The WIR projection preserves all control flow, signed comparisons (`le_i32`, `lt_i32`, `gt_i32`), external calls (`getenv`, `atoi`), and variable mutations exactly as defined in the source.
- WIR-to-raw-LLVM semantic preservation: PASS. The raw LLVM IR maps the WIR 1:1 using `alloca` for local variables, `icmp sle/sgt/slt` for signed comparisons, and correct calls to `@getenv` and `@atoi`.
- Raw LLVM validity, SSA, types, and control flow: PASS. The raw LLVM IR is valid SSA. All `alloca` instructions are properly typed, loads/stores match pointer types, and basic blocks are well-formed with explicit `br` and `ret` instructions.
- Optimized LLVM semantic preservation: PASS. The optimized LLVM inlines `@fib` into `@main`, promotes all stack variables to SSA registers, and uses `freeze i32` to safely handle potential uninitialized/poison values from `atoi`. The logic correctly clamps the input to 10 if `getenv` returns null or if `atoi` returns a value > 46.
- Integer signedness, overflow, shifts, and comparisons: PASS. The source uses 32-bit signed integers. The optimized LLVM correctly uses `icmp ugt i32 %t3.fr, 46` to check the upper bound (safe for positive values) and `icmp ult i32 %t3.fr, 2` to check the `n <= 1` base case. The final assembly uses `cmpl $46, %eax` followed by `ja` (unsigned above) and `cmpl $2, %eax` followed by `jb` (unsigned below), which perfectly matches the optimized LLVM semantics.
- Calls, return values, ABI, stack alignment, and register use: PASS. The x86_64 SysV ABI is strictly followed. `getenv` and `atoi` are called with the pointer argument in `%rdi`. Return values are expected in `%rax`. `main` saves `%rbx` (callee-saved) on entry and restores it before returning, maintaining stack alignment.
- Memory safety, lifetime, leaks, and undefined behavior: PASS. There are no heap allocations or leaks. The `freeze` instruction in optimized LLVM prevents undefined behavior from propagating poison values. The string literal `WEAVE_AUDIT_N` is properly null-terminated in the `.rodata` section.
- Target compatibility and native instruction validity: PASS. The target triple is `x86_64-pc-linux-gnu`. The assembly uses standard x86_64 instructions (`leaq`, `callq`, `testq`, `cmpl`, `addl`, `jle`) and is correctly linked against glibc PLT stubs.
- Native runtime cases and expected observable behavior: PASS. The runtime matrix executes the exact linked artifact (SHA `8096a9e8...`). All 9 cases pass, including missing input, 0, 1, 2, 10, 12, -1, 47, and non-numeric "abc".
- Compiler-generated overhead remaining in final native code: PASS. The final assembly contains no unnecessary stack traffic or redundant instructions. The loop is compact (6 instructions) and uses only registers. The only stack operation is the required `pushq %rbx` for callee-saved register preservation.

## Blocking findings
None found.

## Non-blocking opportunities
- The loop-vectorize and slp-vectorizer passes missed vectorizing the Fibonacci loop, which is expected because Fibonacci is an inherently sequential recurrence relation. This is a source algorithm limitation, not compiler-generated overhead.
- The `freeze i32 %t3` in the optimized LLVM is a defensive measure to handle potential `undef` from `atoi`, which adds a minor instruction. This is a safe compiler choice and does not represent avoidable overhead.

## Suggested verification
- Add a runtime case for `WEAVE_AUDIT_N=46` (max valid input) to explicitly verify the upper boundary condition and ensure the `ja 0x117b` branch correctly avoids the fallback for exactly 46.
- Add a runtime case for `WEAVE_AUDIT_N=2147483648` (integer overflow for `atoi`) to verify how the system handles values outside the 32-bit signed integer range, though this depends on libc `atoi` behavior.
