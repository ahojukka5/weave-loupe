...
- **Audit timestamp (UTC):** `2026-07-28T03:46:10+00:00`
- **Re-audit no later than (UTC):** `2026-08-27T03:46:10+00:00`
...
- **Audited source Git SHA:** `fc6304930daf335cb69c9b5a71dcf8209b6470c9`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `fc6304930daf335cb69c9b5a71dcf8209b6470c9`
- **Auditor content SHA-256:** `4d73e55eabea872158a0b1199256d61044ac90782b33f3159bbe7354a8722870`
...
- **GitHub run ID:** `30326774891`
- **GitHub workflow SHA:** `8b11f9085bba4d3776b0a9f9303979430a117955`
...
- `build_manifest` — SHA-256 `b9f13cd0c22838fc62f2eed9f754389e62a9082dc6fe97334b5388df728caac6`
...
- `llvm` — SHA-256 `0b9610097ae9ad3eba3edf5696406e73fee9c1eca19240c1dbba4c19060dbdcd`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-b6V7CP/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-74ijlwxj/.audit.loupe.upn4gmf2/artifacts/program",
...
## Summary
The final native program is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The source semantics define a runtime-driven Fibonacci computation with input clamping, which is preserved through the WIR and raw LLVM. The optimizer successfully inlines the `fib` function, promotes stack locals to SSA registers, and folds the redundant `n < 0` check (since `atoi` returns a signed integer but the upper-bound check `n > 46` implicitly handles the clamping logic in the optimized IR). The final x86_64 assembly correctly implements the loop using 32-bit signed arithmetic and comparisons, adheres to the System V ABI, and exhibits no unnecessary overhead. All 9 configured runtime matrix cases pass with exact expected exit codes.

## Verification matrix
- Source semantics and expected result: PASS - Source defines `fib` and `main` with input retrieval via `getenv`/`atoi`, clamping to 10 if out of 0..46, and returning `fib(n)`. Runtime cases confirm expected results.
- Weave-to-WIR semantic preservation: PASS - WIR projection matches source structure, externs, types, and control flow exactly.
- WIR-to-raw-LLVM semantic preservation: PASS - Raw LLVM implements WIR operations with correct types (`i32`, `ptr`), signed comparisons (`icmp sle`, `slt`, `sgt`), and control flow.
- Raw LLVM validity, SSA, types, and control flow: PASS - Raw LLVM is valid SSA with proper `alloca`/`store`/`load` patterns, well-formed basic blocks, and correct `ret` instructions.
- Optimized LLVM semantic preservation: PASS - Optimized LLVM inlines `fib`, promotes locals to `phi` nodes, and preserves the loop semantics. The `n < 0` check is subsumed by the `ugt i32 %t3.fr, 46` check, which is safe because values `> 46` and negative values (when interpreted as unsigned due to `atoi`'s `i32` range) both route to the default `n = 10` path.
- Integer signedness, overflow, shifts, and comparisons: PASS - 32-bit signed arithmetic is used. `cmpl $0x2e, %eax` correctly implements the upper bound check. Loop counter uses `incl` and `cmpl` with `jle`.
- Calls, return values, ABI, stack alignment, and register use: PASS - `getenv` and `atoi` are called via PLT with arguments in `%rdi`. `%rbx` is callee-saved and correctly pushed/popped. Stack is 16-byte aligned at call sites.
- Memory safety, lifetime, leaks, and undefined behavior: PASS - No dynamic allocation. String literal is properly null-terminated. No out-of-bounds access. `freeze` is used in optimized LLVM to prevent poison propagation.
- Target compatibility and native instruction validity: PASS - Target is `x86_64-pc-linux-gnu`. Instructions are standard x86_64. Linked executable contains standard CRT sections.
- Native runtime cases and expected observable behavior: PASS - All 9 runtime cases pass with expected exit codes (0, 1, 55, 144).
- Compiler-generated overhead remaining in final native code: PASS - No overhead remains. `fib` is inlined. Stack traffic is eliminated. Loop is compact with no spills.

## Blocking findings
None found.

## Non-blocking opportunities
- The optimized LLVM replaces the signed `n < 0` check with an unsigned `ugt i32 %t3.fr, 46` check. This is semantically equivalent for the clamping logic because any negative `i32` has a bit pattern that is unsigned-greater-than 46. This is a valid optimization, not a defect.
- The loop in `main` is not vectorized, as noted in the optimization record. This is expected for a recurrence relation like Fibonacci and does not represent compiler-generated overhead.

## Suggested verification
- Add a runtime case for `WEAVE_AUDIT_N=46` to explicitly test the boundary condition of the upper clamp.
- Add a runtime case for `WEAVE_AUDIT_N=2147483648` (if `atoi` supports it) to verify large positive overflow behavior, though `atoi` typically caps at `INT_MAX`.
