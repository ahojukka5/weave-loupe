...
- **Report content SHA-256:** `2c52b435563b35c529569bf14e994e93f786aa7573a9763fca54fcc96cb2a7bc`
- **Audit timestamp (UTC):** `2026-07-28T11:29:54+00:00`
- **Re-audit no later than (UTC):** `2026-08-27T11:29:54+00:00`
...
- **Audited source Git SHA:** `59e6c18677090badb147b204c34035d8dc9041bb`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `59e6c18677090badb147b204c34035d8dc9041bb`
- **Auditor content SHA-256:** `0c0ab07c66782353d7bae784e08447a77ba29f3b703ca83a06c9f1cf13af7541`
...
- **Provider response ID:** `chatcmpl-89b06a78-d793-4067-a59d-c6ea1f73aaa6`
- **Provider system fingerprint:** `unavailable`
- **GitHub run ID:** `30354933348`
- **GitHub workflow SHA:** `3a9e5e1ddc901d198b9c06c1347c99877891794e`
...
- `build_manifest` — SHA-256 `a99dadd89f20f9b4643d6ecd3155fb4937d1bf3bebc77f01507b4695bc413ef6`
...
- `llvm` — SHA-256 `472cfdd6eb5580ee912b05c47a80c9ca8d059f6fc0260a24741d1e65ce115eaf`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-YTcIRf/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-9lbou3y6/.audit.loupe.qcs97hj0/artifacts/program",
...
## Summary
The final native program for `docs/audit/fibonacci_runtime.weave` is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The audit traces the source semantics through cleaned WIR, raw LLVM, optimized LLVM, target assembly, linked executable disassembly, and direct runtime matrix evidence. All 9 runtime cases passed, matching expected exit codes. The optimized LLVM successfully inlines the `fib` function and eliminates stack traffic, resulting in a compact native loop that preserves the input-dependent Fibonacci computation. No merge-blocking defects were found.

## Verification matrix
- Source semantics and expected result: PASS. The source defines a program that reads `WEAVE_AUDIT_N`, defaults to 10 if missing/out-of-range, and computes the Nth Fibonacci number. The runtime matrix in the analysis JSON confirms all 9 cases passed with expected exit codes (e.g., `WEAVE_AUDIT_N=10` yields exit code 55, `WEAVE_AUDIT_N=12` yields 144).
- Weave-to-WIR semantic preservation: PASS. The WIR review projection preserves all source constructs: `getenv`/`atoi` externs, `fib` function with `le_i32` guard and `while` loop, and `main` with input validation (`lt_i32`, `gt_i32`) and fallback to `const_i32 10`.
- WIR-to-raw-LLVM semantic preservation: PASS. Raw LLVM IR declares `getenv`/`atoi`, defines `@main` and internal `@fib` with matching control flow (`icmp sle`, `while.cond1` loop) and arithmetic (`add i32`).
- Raw LLVM validity, SSA, types, and control flow: PASS. The raw LLVM uses valid SSA form with typed temporaries, proper `alloca`/`store`/`load` sequences, and well-formed branches. The analysis JSON confirms 0 `undef_uses` and 0 `poison_uses`.
- Optimized LLVM semantic preservation: PASS. The optimized LLVM inlines `fib` into `main`, promotes all stack variables to SSA `phi` nodes, and preserves the input-dependent loop. The `freeze i32` on `atoi`'s return safely handles potential `undef` from uninitialized bytes.
- Integer signedness, overflow, shifts, and comparisons: PASS. The source restricts input to 0..46, preventing 32-bit overflow (max Fib(46) = 1836311903). The optimized LLVM uses `icmp ult` for the upper bound check (safe because `freeze` prevents `undef` poisoning) and `icmp ult` for the `n < 2` check, matching the source's signed semantics for valid inputs.
- Calls, return values, ABI, stack alignment, and register use: PASS. The disassembly shows `main` aligns the stack via `pushq %rbx` (8 bytes), calls `getenv@PLT`/`atoi@PLT` with arguments in `%rdi`, and returns the result in `%eax`, conforming to the x86-64 SysV ABI.
- Memory safety, lifetime, leaks, and undefined behavior: PASS. The program uses only stack scalars and read-only string literals. No dynamic allocation is performed. The `freeze` instruction in optimized LLVM prevents UB from `atoi`'s potential `undef` return on non-numeric input.
- Target compatibility and native instruction validity: PASS. The target assembly specifies `x86_64-pc-linux-gnu` and uses standard instructions (`leaq`, `callq`, `cmpl`, `jle`). The linked executable is `elf64-x86-64`, matching the machine architecture.
- Native runtime cases and expected observable behavior: PASS. The analysis JSON runtime matrix shows all 9 cases passed, including missing input, zero, one, two, ten, twelve, negative, too-large, and non-numeric inputs.
- Compiler-generated overhead remaining in final native code: PASS. The final native code is compact (25 instructions in `main`). The loop contains only 7 instructions (`movl`, `addl`, `incl`, `cmpl`, `jle`) with no unnecessary spills or stack traffic. The optimization record confirms the loop was not vectorized due to the recurrence, which is expected for Fibonacci.

## Blocking findings
None found.

## Non-blocking opportunities
- The Fibonacci loop is not vectorized due to the loop-carried dependency, as noted in the optimization record. This is a source algorithm cost, not compiler-generated overhead. No native code improvement is possible without changing the source algorithm.
- The raw LLVM uses `alloca`/`store`/`load` for local variables, but these are fully optimized away in the optimized LLVM and final native code. This is not a defect.

## Suggested verification
- Add a runtime case for `WEAVE_AUDIT_N=46` to verify the maximum valid Fibonacci value (1836311903) and ensure no 32-bit overflow occurs at the boundary.
- Add a runtime case for an empty string (`WEAVE_AUDIT_N=""`) to further verify `atoi` behavior and `freeze` safety.
