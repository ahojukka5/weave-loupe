...
- **Report content SHA-256:** `a426720e68a17b605c52e28b49dc4bbb931a996fbb11bc7c565f8af68278bdf4`
- **Audit timestamp (UTC):** `2026-07-28T08:06:48+00:00`
- **Re-audit no later than (UTC):** `2026-08-27T08:06:48+00:00`
...
- **Audited source Git SHA:** `1dc7c447a8bcc60b6e4c3c664266533e08cd7582`
- **Source tree state:** `clean`
- **Weave Loupe Git SHA:** `1dc7c447a8bcc60b6e4c3c664266533e08cd7582`
- **Auditor content SHA-256:** `3aeb9100241bb4a32b6fe4ced5cd7cc7a4ef1732273faa99235e55c323783618`
...
- **GitHub run ID:** `30340910680`
- **GitHub workflow SHA:** `e9ad5b552cfd8a7c494938436618f97f6c826966`
...
- `build_manifest` — SHA-256 `3cd95ddd8137c9870ae2eb6753f5643cb1205026e9023b1f695d4e749874b92e`
...
- `llvm` — SHA-256 `ea7acf68bcad1496a51085792f666693e470f30170663e23680f264c7c2f0bd8`
...
```llvm
; generated by weavec
; source: /tmp/weavec-build-dgC4qF/program.wir
; core-version: 2
...
```json
{
  ...
  "output": "/tmp/loupe-audit-0j00oez2/.audit.loupe.sx92pcyz/artifacts/program",
...
## Summary
The final native program for `fibonacci_runtime.weave` is correct, safe, ABI-valid, target-compatible, and free from avoidable compiler-generated overhead. The Weave source defines a runtime Fibonacci calculator that reads `WEAVE_AUDIT_N`, clamps the value to `[0, 46]` (defaulting to 10 if out of range or missing), and computes the nth Fibonacci number iteratively. The cleaned WIR preserves these semantics exactly. Raw LLVM IR correctly lowers the WIR to valid SSA with proper signed integer comparisons and memory operations. During LLVM optimization, the `fib` function is inlined into `main`, and the input validation logic is partially optimized: the `lt_i32 n 0` check is eliminated because the subsequent `ugt i32 %t3.fr, 46` unsigned comparison correctly handles negative numbers by treating them as large unsigned values, falling back to 10. The final x86_64 assembly is compact, uses correct signed comparisons (`cmpl`/`jle`) for the loop, adheres to the System V ABI, and executes successfully across all 9 runtime matrix cases.

## Verification matrix
- Source semantics and expected result: PASS. The source specifies reading an environment variable, clamping to `[0, 46]` with a fallback to 10, and computing Fibonacci. Runtime cases confirm expected outputs (e.g., 10->55, 12->144, 47->55).
- Weave-to-WIR semantic preservation: PASS. The WIR projection matches the source structure, including extern declarations, `if` conditions, `while` loop, and variable updates.
- WIR-to-raw-LLVM semantic preservation: PASS. Raw LLVM IR allocates stack slots for variables, uses `icmp sle`/`slt`/`sgt` for signed comparisons, and correctly structures control flow with branches to match WIR semantics.
- Raw LLVM validity, SSA, types, and control flow: PASS. The IR is valid SSA, uses correct types (`i32`, `ptr`), and has well-formed basic blocks with proper branching and returns.
- Optimized LLVM semantic preservation: PASS. The optimized IR inlines `fib`, promotes stack variables to registers via PHI nodes, and uses `icmp ugt` for the upper bound check, which is semantically equivalent to the original signed checks for the valid input domain.
- Integer signedness, overflow, shifts, and comparisons: PASS. The source uses 32-bit signed integers. The maximum computed value is `fib(46) = 1836311903`, which fits within a 32-bit signed integer. The assembly uses `cmpl`/`jle` for the loop condition, preserving signed semantics.
- Calls, return values, ABI, stack alignment, and register use: PASS. The assembly correctly uses `rdi` for the first argument to `getenv` and `atoi`, saves `rbx` with `pushq`, aligns the stack to 16 bytes, and returns the result in `eax`.
- Memory safety, lifetime, leaks, and undefined behavior: PASS. The program uses a string literal for `getenv`, passes the returned pointer directly to `atoi`, and has no dynamic memory allocation. No undefined behavior is present.
- Target compatibility and native instruction validity: PASS. The assembly targets `x86_64-pc-linux-gnu` and uses standard instructions valid for the specified Intel Xeon processor.
- Native runtime cases and expected observable behavior: PASS. All 9 runtime cases passed, matching expected exit codes with empty stdout/stderr.
- Compiler-generated overhead remaining in final native code: PASS. The final assembly is compact with no unnecessary spills or redundant instructions. The loop is tight and efficiently scheduled.

## Blocking findings
None found.

## Non-blocking opportunities
- The `fib` function is not vectorized, as noted in the optimization record. This is expected for a loop-carried dependency and is not a compiler-generated overhead defect.
- The raw LLVM IR uses stack allocations (`alloca`) for all local variables, which are optimized away in the final code. This is standard compiler behavior and not a defect.

## Suggested verification
- Add a runtime case for `WEAVE_AUDIT_N=46` to directly verify the upper boundary of the Fibonacci computation without triggering the fallback.
- Add a runtime case for a large positive number (e.g., `WEAVE_AUDIT_N=100`) to ensure the fallback to 10 works correctly for values that would overflow a 32-bit integer if computed.
